...
// Fetch data from NMIMS University API
async function fetchData() {
  try {
    const res = await fetch("https://netcomindia.xyz/api/nmims-university", {
      next: { revalidate: 0 }, // Fetch fresh data on every request
    });
    if (!res.ok) throw new Error("Failed to fetch nmims-university data");
    return res.json();
  } catch (error) {
...
// Static brochure paths (fallback if not provided by API)
const brochurePaths = {
  "Online MBA": "/assets/brochure/NMIMS-MBA/NMIMS-Online-MBA-Brochure.pdf",
  "Online BBA": "/assets/brochure/NMIMS-BBA/UG_Brochure_A224.pdf",
  "Online BCOM": "/assets/brochure/NMIMS-B.COM/NMIMS-Online-MBA-Brochure.pdf",
  "Online DIPLOMA": "/assets/brochure/Diploma/Diploma-and-Certificate-Program_V1.pdf",
};
...
export default function Page() {
  ...
  useEffect(() => {
    const loadCourses = async () => {
      ...
      if (data) {
        // Map API data to courseSpecializations object
        const specializations = {};
        const courseList = data.map((course) => ({
          name: course.name,
          specializations: course.specializations,
          feeRange: getFeeRange(course.specializations),
          imageSrc: `/assets/img/universities/${course.name.replace(/ /g, "_").toUpperCase()}.png` || "/assets/img/universities/default.png",
          brochure: course.brochure || brochurePaths[course.name] || "/assets/brochure/default-brochure.pdf",
        }));
        data.forEach((course) => {
          specializations[course.name] = course.specializations;
        });
        setCourses(courseList);
        setCourseSpecializations(specializations);
      } else {
  ...
  const getFeeRange = (specializations) => {
    const fees = specializations?.map((spec) => spec.fees) || [];
  ...
  return (
    ...
    `}
                  ...
                  <div className="collegeDetails_maxWidth__6vBVL" id="High">
                    <div className="Highlights_container__yqw8uelan__t">
                      <h2
                        style={{
                          fontSize: "24px",
                          margin: "20px 0",
