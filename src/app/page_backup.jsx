  ...
  const handleOpenBrochureModal = async (url, university) => {
    try {
      const response = await fetch(
        "https://netcomindia.xyz/api/brochure-downloads",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ url, university }),
        }
      );
      ...
      if (response.ok && data.status === "success") {
        ...
        alert("Failed to store brochure data. Please try again.");
      }
    } catch (error) {
      console.error("Error storing brochure data:", error);
