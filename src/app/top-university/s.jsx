...
export default function Page() {
  ...
  // Define rawUniversities without url fields (except where already present)
  ...
  // Dynamically add url to all universities
  const universities = rawUniversities.map((uni) => ({
    ...uni,
    url:
      uni.url || // Preserve existing url if present (e.g., for Amity)
      `/college/${uni.name
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, "")
        .replace(/\s+/g, "-")}`,
  ...
  // Define filter options (unchanged)
      ...
      <div>
        <div className="bestcollege_main__G_q2G">
          <img
            ...
            fetchPriority="high" // Fixed from fetchPriority to fetchPriority
            width={1920}
            height={870}
            decoding="async"
            data-nimg={1}
