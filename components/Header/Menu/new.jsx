<div>
  <meta charSet="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <link
    rel="preload"
    href="/_next/static/media/6905431624c34d00-s.p.woff2"
    as="font"
    crossOrigin
    type="font/woff2"
  />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2Fimages%2Flogo-blue.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Flogo-blue.webp&w=1920&q=75 2x"
    fetchpriority="high"
  />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2FcollegeIconImg%2Famity.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Famity.png&w=384&q=75 2x"
    fetchpriority="high"
  />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2FcollegeIconImg%2Fcu.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fcu.png&w=384&q=75 2x"
    fetchpriority="high"
  />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2FcollegeIconImg%2Fdypatil.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fdypatil.png&w=384&q=75 2x"
    fetchpriority="high"
  />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2Fimages%2Fslidercards%2F1.png&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fslidercards%2F1.png&w=1920&q=75 2x"
    fetchpriority="high"
  />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2Fimages%2Fslidercards%2F2.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fslidercards%2F2.webp&w=1920&q=75 2x"
    fetchpriority="high"
  />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2Fimages%2Fslidercards%2F3.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fslidercards%2F3.webp&w=1920&q=75 2x"
    fetchpriority="high"
  />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2Fimages%2Fwhyus1.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fwhyus1.webp&w=1080&q=75 2x"
    fetchpriority="high"
  />
  <link rel="preload" as="image" href="/HomePage/Kartikeya.webp" />
  <link rel="preload" as="image" href="/HomePage/Manav-Dhodare.webp" />
  <link
    rel="stylesheet"
    href="/_next/static/css/bcdb44b6ad772c90.css"
    data-precedence="next"
  />
  <link
    rel="stylesheet"
    href="/_next/static/css/a50d1454e259970b.css"
    data-precedence="next"
  />
  <link
    rel="stylesheet"
    href="/_next/static/css/04bdd9c7a469b901.css"
    data-precedence="next"
  />
  <link
    rel="stylesheet"
    href="/_next/static/css/ecbb68b163419596.css"
    data-precedence="next"
  />
  <link
    rel="stylesheet"
    href="/_next/static/css/afc7461932b964d5.css"
    data-precedence="next"
  />
  <link
    rel="stylesheet"
    href="/_next/static/css/3a6b4218bb14b3ef.css"
    data-precedence="next"
  />
  <link
    rel="stylesheet"
    href="/_next/static/css/e881ba373a433cf6.css"
    data-precedence="next"
  />
  <link
    rel="stylesheet"
    href="/_next/static/css/35079bfe8c614826.css"
    data-precedence="next"
  />
  <link
    rel="stylesheet"
    href="/_next/static/css/cc9687947ca46cf2.css"
    data-precedence="next"
  />
  <link
    rel="preload"
    as="script"
    fetchpriority="low"
    href="/_next/static/chunks/webpack-7d76c02a16c1faae.js"
  />
  <link
    rel="preload"
    href="https://www.googletagmanager.com/gtag/js?id=G-WD1644S6QD"
    as="script"
  />
  <link rel="preload" as="image" href="/HomePage/Namita.webp" />
  <link rel="preload" as="image" href="/HomePage/Piyush.webp" />
  <link rel="preload" as="image" href="/HomePage/bhavika.webp" />
  <link
    rel="preload"
    as="image"
    imagesrcset="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs4.672ee0b2.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs4.672ee0b2.webp&w=1200&q=75 2x"
  />
  <meta
    name="robots"
    content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1"
  />
  <title>Best Education Consultants in India | Learning Routes</title>
  <meta
    name="description"
    content="Want to achieve your career goals? Unlock your potential with Learning Routes - our experts will guide you towards a brighter future."
  />
  <link rel="canonical" href="https://www.learningroutes.in" />
  <link rel="icon" href="/favicon.ico" type="image/x-icon" sizes="36x36" />
  <meta name="next-size-adjust" />
  <noscript>
    &lt;iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TFBF3W7L"
    height="0" width="0" style="display:none;visibility:hidden"
    &gt;&lt;/iframe&gt;
  </noscript>
  <div>
    <div className="TopNavbar_topNavBar__Container__63eNB">
      <div className="MobileNav_mobileNav__9hkbo">
        <div>
          <a href="/">
            <img
              alt="icon"
              loading="lazy"
              width={635}
              height={213}
              decoding="async"
              data-nimg={1}
              className="MobileNav_mobileNavLogoImg__uxQ7p"
              style={{ color: "transparent" }}
              srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Flogo-blue.7ec37570.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Flogo-blue.7ec37570.webp&w=1920&q=75 2x"
              src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Flogo-blue.7ec37570.webp&w=1920&q=75"
            />
          </a>
        </div>
        <div>
          <div>
            <div>
              <button className="MobileNav_menu__xxzsj">
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  strokeWidth={0}
                  viewBox="0 0 512 512"
                  height="1em"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M32 96v64h448V96H32zm0 128v64h448v-64H32zm0 128v64h448v-64H32z" />
                </svg>
              </button>
            </div>
          </div>{" "}
        </div>
      </div>
      <div className="TopNavbar_topNavBar__flexContainer__jpTqI">
        <div className="TopNavbar_topNavBar__linksContainer__5ci9a">
          <div className="TopNavbar_topNavBar__link__Y5UGY">
            <a className="TopNavbar_linkComp__iHTrA" href="/colleges">
              <p>Top Universities &amp; Colleges</p>
            </a>
          </div>
          <div className="TopNavbar_topNavBar__link__Y5UGY">
            <a className="TopNavbar_linkComp__iHTrA" href="/courses">
              <p>Trending Courses</p>
            </a>
          </div>
          <div className="TopNavbar_topNavBar__link__Y5UGY">
            <a className="TopNavbar_linkComp__iHTrA" href="/study-abroad">
              <p>Study Abroad</p>
            </a>
          </div>
          <div className="TopNavbar_topNavBar__link__Y5UGY">
            <a className="TopNavbar_linkComp__iHTrA" href="/exams">
              <p>Entrance Exams</p>
            </a>
          </div>
          <div className="TopNavbar_topNavBar__link__Y5UGY">
            <a className="TopNavbar_linkComp__iHTrA" href="/web-stories">
              <p>Web Stories</p>
            </a>
          </div>
        </div>
        <div className="TopNavbar_topNavBar__loginContainer__d8NFn">
          <div className="TopNavbar_phoneBtn__TCkgd">
            {" "}
            <svg
              stroke="currentColor"
              fill="currentColor"
              strokeWidth={0}
              viewBox="0 0 256 256"
              fontSize="1.3rem"
              height="1em"
              width="1em"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M152.27,37.93a8,8,0,0,1,9.8-5.66,86.22,86.22,0,0,1,61.66,61.66,8,8,0,0,1-5.66,9.8A8.23,8.23,0,0,1,216,104a8,8,0,0,1-7.73-5.93,70.35,70.35,0,0,0-50.33-50.34A8,8,0,0,1,152.27,37.93Zm-2.33,41.8c13.79,3.68,22.65,12.55,26.33,26.34A8,8,0,0,0,184,112a8.23,8.23,0,0,0,2.07-.27,8,8,0,0,0,5.66-9.8c-5.12-19.16-18.5-32.54-37.66-37.66a8,8,0,1,0-4.13,15.46Zm72.43,78.73-47.11-21.11-.13-.06a16,16,0,0,0-15.17,1.4,8.12,8.12,0,0,0-.75.56L134.87,160c-15.42-7.49-31.34-23.29-38.83-38.51l20.78-24.71c.2-.25.39-.5.57-.77a16,16,0,0,0,1.32-15.06l0-.12L97.54,33.64a16,16,0,0,0-16.62-9.52A56.26,56.26,0,0,0,32,80c0,79.4,64.6,144,144,144a56.26,56.26,0,0,0,55.88-48.92A16,16,0,0,0,222.37,158.46Z" />
            </svg>{" "}
            <a
              href="tel:+919228165349"
              style={{ textDecoration: "none", color: "var(--dark-blue)" }}
            >
              9228165349
            </a>
          </div>
          <div className="TopNavbar_topNavBar__login__NTr1D">
            <svg
              stroke="currentColor"
              fill="currentColor"
              strokeWidth={0}
              viewBox="0 0 448 512"
              className="TopNavbar_topNavBar__login_icon__DYr7p"
              height="1em"
              width="1em"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M313.6 304c-28.7 0-42.5 16-89.6 16-47.1 0-60.8-16-89.6-16C60.2 304 0 364.2 0 438.4V464c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48v-25.6c0-74.2-60.2-134.4-134.4-134.4zM400 464H48v-25.6c0-47.6 38.8-86.4 86.4-86.4 14.6 0 38.3 16 89.6 16 51.7 0 74.9-16 89.6-16 47.6 0 86.4 38.8 86.4 86.4V464zM224 288c79.5 0 144-64.5 144-144S303.5 0 224 0 80 64.5 80 144s64.5 144 144 144zm0-240c52.9 0 96 43.1 96 96s-43.1 96-96 96-96-43.1-96-96 43.1-96 96-96z" />
            </svg>
            <div>
              <p className="TopNavbar_topNavBar__login_text__Y0ErT">
                Loading...
              </p>
            </div>
          </div>
          <a
            className="TopNavbar_topNavBar__freeCoursesBtn__48l_Q"
            href="/freecourses"
          >
            Free Courses
          </a>
        </div>
      </div>
    </div>
    <div className="MainNavbar_mainNavBar__container__WxyCm">
      <div className="MainNavbar_mainNavBar__flexContainer__uxuMS">
        <div>
          <a href="/">
            <img
              alt="logo"
              fetchpriority="high"
              width={635}
              height={213}
              decoding="async"
              data-nimg={1}
              className="MainNavbar_mainNavBar__image__ZE0A9"
              style={{ color: "transparent" }}
              srcSet="/_next/image?url=%2Fimages%2Flogo-blue.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Flogo-blue.webp&w=1920&q=75 2x"
              src="/_next/image?url=%2Fimages%2Flogo-blue.webp&w=1920&q=75"
            />
          </a>
        </div>
        <div className="MainNavbar_mainNavBar__navbarContainer__8aIZC">
          <div className="MainNavbar_mainNavBar__exploreButton__llabC">
            <div>
              <div className="Modal_ModalContainer__yYgZo">
                <div className="Modal_buttonXl__4JKO_">
                  Explore Courses
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 24 24"
                    style={{ marginLeft: 4 }}
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M16.293 9.293 12 13.586 7.707 9.293l-1.414 1.414L12 16.414l5.707-5.707z" />
                  </svg>
                </div>
              </div>
            </div>
          </div>
          <div className="MainNavbar_mainNavBar__buttonsContainer__QAT3r">
            <div className="MainNavbar_mainNavBar__topUniversities___6bwc">
              <div className="Modal_ModalContainer__yYgZo">
                <div className="Modal_buttonXl__4JKO_">
                  Top University
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 24 24"
                    style={{ marginLeft: 4 }}
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M16.293 9.293 12 13.586 7.707 9.293l-1.414 1.414L12 16.414l5.707-5.707z" />
                  </svg>
                </div>
              </div>
            </div>
            <div className="MainNavbar_mainNavBar__buttons__fxxQT">
              <a className="MainNavbar_link__Je6tm" href="/ai-college-finder">
                College Finder{/* */}{" "}
                <span className="MainNavbar_glowEffect__ORd8S">AI Based</span>
              </a>
            </div>
            <div className="MainNavbar_mainNavBar__buttons__fxxQT">
              <a className="MainNavbar_link__Je6tm" href="/online-video-meet">
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  strokeWidth={0}
                  viewBox="0 0 1024 1024"
                  className="MainNavbar_flashingIcon__290hO"
                  height="1em"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M912 302.3L784 376V224c0-35.3-28.7-64-64-64H128c-35.3 0-64 28.7-64 64v576c0 35.3 28.7 64 64 64h592c35.3 0 64-28.7 64-64V648l128 73.7c21.3 12.3 48-3.1 48-27.6V330c0-24.6-26.7-40-48-27.7zM712 792H136V232h576v560zm176-167l-104-59.8V458.9L888 399v226zM208 360h112c4.4 0 8-3.6 8-8v-48c0-4.4-3.6-8-8-8H208c-4.4 0-8 3.6-8 8v48c0 4.4 3.6 8 8 8z" />
                </svg>{" "}
                Free Counselling
              </a>
            </div>
            <div className="MainNavbar_mainNavBar__buttons__fxxQT">
              <a className="MainNavbar_link__Je6tm" href="/career">
                Career
              </a>
            </div>
            <div className="MainNavbar_mainNavBar__buttons__fxxQT">
              <a className="MainNavbar_link__Je6tm" href="/blog">
                Blogs
              </a>
            </div>
            <div className="MainNavbar_mainNavBar__buttons__fxxQT">
              <a className="MainNavbar_link__Je6tm" href="/contact-us">
                Contact Us
              </a>
            </div>
            <div className="MainNavbar_mainNavBar__referral__wb9C6">
              <a href="/refer-and-earn">
                <div className="MainNavbar_tooltip__r7Ode">
                  <img
                    alt="img"
                    loading="lazy"
                    width={24}
                    height={25}
                    decoding="async"
                    data-nimg={1}
                    style={{ color: "transparent" }}
                    srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FReferral.e77cea67.png&w=32&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2FReferral.e77cea67.png&w=48&q=75 2x"
                    src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FReferral.e77cea67.png&w=48&q=75"
                  />
                  <span className="MainNavbar_tooltipText__Mu_xF">
                    Refer and Earn{/* */}{" "}
                    <span className="MainNavbar_rupee__AoHAZ">₹ 5000</span>
                  </span>
                </div>
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <a
    className="float"
    target="_blank"
    href="https://api.whatsapp.com/send?phone=919667063002&text=I'm%20looking%20for"
  >
    <svg
      stroke="currentColor"
      fill="currentColor"
      strokeWidth={0}
      viewBox="0 0 448 512"
      style={{ color: "white" }}
      className="my_float"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z" />
    </svg>
  </a>
  {/*$*/}
  <div>
    <button
      className="enquirynow_enquirynow_btn__mRuEZ"
      style={{ transformOrigin: "bottom right" }}
    >
      <svg
        stroke="currentColor"
        fill="currentColor"
        strokeWidth={0}
        viewBox="0 0 512 512"
        height="1em"
        width="1em"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path d="M256 8C119.043 8 8 119.083 8 256c0 136.997 111.043 248 248 248s248-111.003 248-248C504 119.083 392.957 8 256 8zm0 448c-110.532 0-200-89.431-200-200 0-110.495 89.472-200 200-200 110.491 0 200 89.471 200 200 0 110.53-89.431 200-200 200zm107.244-255.2c0 67.052-72.421 68.084-72.421 92.863V300c0 6.627-5.373 12-12 12h-45.647c-6.627 0-12-5.373-12-12v-8.659c0-35.745 27.1-50.034 47.579-61.516 17.561-9.845 28.324-16.541 28.324-29.579 0-17.246-21.999-28.693-39.784-28.693-23.189 0-33.894 10.977-48.942 29.969-4.057 5.12-11.46 6.071-16.666 2.124l-27.824-21.098c-5.107-3.872-6.251-11.066-2.644-16.363C184.846 131.491 214.94 112 261.794 112c49.071 0 101.45 38.304 101.45 88.8zM298 368c0 23.159-18.841 42-42 42s-42-18.841-42-42 18.841-42 42-42 42 18.841 42 42z" />
      </svg>{" "}
      Enquire Now
    </button>
  </div>
  {/*/$*/}
  <main className="flex min-h-screen flex-col items-center justify-between p-24">
    <div>
      <div>
        <div className="Hero_relative__x641X">
          <div className="Hero_newheroContainer__lAPRu">
            <div className="swiper">
              <div className="swiper-wrapper">
                <div className="swiper-slide Hero_card__ynLLv">
                  <div className="Hero_heroMain__dP9sl">
                    <div>
                      <div className="Hero_searchContent__wSOwy">
                        <div className="Hero_searchContentBox__mTqoR">
                          <div>
                            <div className="counter_countercontainer__CiYqi">
                              <div className="counter_counter__W_2Wz">
                                <div className="counter_digit__L6cff">1</div>
                                <div className="counter_digit__L6cff">6</div>
                                <div className="counter_digit__L6cff">5</div>
                                <div className="counter_digit__L6cff">8</div>
                                <div className="counter_digit__L6cff">5</div>
                                <div className="counter_digit__L6cff">7</div>
                                <div className="counter_digit__L6cff">4</div>
                                <div className="counter_livetextbanner__Vykhp">
                                  · LIVE
                                </div>
                              </div>
                              <div className="counter_counterlabel__r7_fx">
                                STUDENTS COUNSELLED
                              </div>
                            </div>
                          </div>
                          <div>
                            <div className="SearchBar_searchWrapper__zmgjs">
                              <div className="SearchBar_searchBar__lYNNN">
                                <div
                                  className="SearchBar_Hero_input__VYpfM"
                                  variant="unstyled"
                                >
                                  Search for colleges and courses
                                </div>
                                <div className="SearchBar_Hero_iconContainer__psyvf">
                                  <svg
                                    stroke="currentColor"
                                    fill="none"
                                    strokeWidth={2}
                                    viewBox="0 0 24 24"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    className="SearchBar_Hero_icon__Wmwe2"
                                    height="1em"
                                    width="1em"
                                    xmlns="http://www.w3.org/2000/svg"
                                  >
                                    <circle cx={11} cy={11} r={8} />
                                    <line
                                      x1={21}
                                      y1={21}
                                      x2="16.65"
                                      y2="16.65"
                                    />
                                  </svg>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="Hero_textContainer__zTKJs">
                          <div className="Hero_Hero_text_1__tiOzY">
                            <strong>Grab It Now:</strong> Early Bird Offer!
                          </div>
                          <div className="Hero_Hero_text_2__pG74x">
                            ₹ 7000/- Off On Admissions<sup>*</sup>
                          </div>
                          <div className="Hero_hashtag__ky4R9">
                            <img
                              alt="hashtag"
                              loading="lazy"
                              width={663}
                              height={238}
                              decoding="async"
                              data-nimg={1}
                              style={{ color: "transparent" }}
                              srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fbatch.ac4d2e8b.webp&w=750&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fbatch.ac4d2e8b.webp&w=1920&q=75 2x"
                              src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fbatch.ac4d2e8b.webp&w=1920&q=75"
                            />
                          </div>
                          <div className="Hero_made__edb3d">
                            <div>MADE IN INDIA WITH</div>
                            <div>
                              <svg
                                stroke="currentColor"
                                fill="currentColor"
                                strokeWidth={0}
                                viewBox="0 0 512 512"
                                color="red"
                                style={{ color: "red" }}
                                height="1em"
                                width="1em"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M462.3 62.6C407.5 15.9 326 24.3 275.7 76.2L256 96.5l-19.7-20.3C186.1 24.3 104.5 15.9 49.7 62.6c-62.8 53.6-66.1 149.8-9.9 207.9l193.5 199.8c12.5 12.9 32.8 12.9 45.3 0l193.5-199.8c56.3-58.1 53-154.3-9.8-207.9z" />
                              </svg>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="Hero_imgCont__lhqFP">
                      <img
                        alt="slide1"
                        loading="eager"
                        width={553}
                        height={698}
                        decoding="async"
                        data-nimg={1}
                        style={{ color: "transparent" }}
                        srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs4.672ee0b2.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs4.672ee0b2.webp&w=1200&q=75 2x"
                        src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs4.672ee0b2.webp&w=1200&q=75"
                      />
                    </div>
                  </div>
                </div>
                <div className="swiper-slide Hero_card__ynLLv">
                  <div className="Hero_heroMain__dP9sl">
                    <div>
                      <div className="Hero_searchContent__wSOwy">
                        <div className="Hero_searchContentBox__mTqoR">
                          <div>
                            <div className="counter_countercontainer__CiYqi">
                              <div className="counter_counter__W_2Wz">
                                <div className="counter_digit__L6cff">1</div>
                                <div className="counter_digit__L6cff">6</div>
                                <div className="counter_digit__L6cff">5</div>
                                <div className="counter_digit__L6cff">8</div>
                                <div className="counter_digit__L6cff">5</div>
                                <div className="counter_digit__L6cff">7</div>
                                <div className="counter_digit__L6cff">4</div>
                                <div className="counter_livetextbanner__Vykhp">
                                  · LIVE
                                </div>
                              </div>
                              <div className="counter_counterlabel__r7_fx">
                                STUDENTS COUNSELLED
                              </div>
                            </div>
                          </div>
                          <div>
                            <div className="SearchBar_searchWrapper__zmgjs">
                              <div className="SearchBar_searchBar__lYNNN">
                                <div
                                  className="SearchBar_Hero_input__VYpfM"
                                  variant="unstyled"
                                >
                                  Search for colleges and courses
                                </div>
                                <div className="SearchBar_Hero_iconContainer__psyvf">
                                  <svg
                                    stroke="currentColor"
                                    fill="none"
                                    strokeWidth={2}
                                    viewBox="0 0 24 24"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    className="SearchBar_Hero_icon__Wmwe2"
                                    height="1em"
                                    width="1em"
                                    xmlns="http://www.w3.org/2000/svg"
                                  >
                                    <circle cx={11} cy={11} r={8} />
                                    <line
                                      x1={21}
                                      y1={21}
                                      x2="16.65"
                                      y2="16.65"
                                    />
                                  </svg>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="Hero_textContainer__zTKJs">
                          <div className="Hero_Hero_text_1__tiOzY">
                            India Ki Top Universities Se
                          </div>
                          <div className="Hero_Hero_text_2__pG74x">
                            Apni Skills Ko Enhance Karo!
                          </div>
                          <div className="Hero_hashtag__ky4R9">
                            <img
                              alt="hashtag"
                              loading="lazy"
                              width={653}
                              height={215}
                              decoding="async"
                              data-nimg={1}
                              style={{ color: "transparent" }}
                              srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fposter1.44f1b3a4.webp&w=750&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fposter1.44f1b3a4.webp&w=1920&q=75 2x"
                              src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fposter1.44f1b3a4.webp&w=1920&q=75"
                            />
                          </div>
                          <div className="Hero_made__edb3d">
                            <div>MADE IN INDIA WITH</div>
                            <div>
                              <svg
                                stroke="currentColor"
                                fill="currentColor"
                                strokeWidth={0}
                                viewBox="0 0 512 512"
                                color="red"
                                style={{ color: "red" }}
                                height="1em"
                                width="1em"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M462.3 62.6C407.5 15.9 326 24.3 275.7 76.2L256 96.5l-19.7-20.3C186.1 24.3 104.5 15.9 49.7 62.6c-62.8 53.6-66.1 149.8-9.9 207.9l193.5 199.8c12.5 12.9 32.8 12.9 45.3 0l193.5-199.8c56.3-58.1 53-154.3-9.8-207.9z" />
                              </svg>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="Hero_imgCont__lhqFP">
                      <a href="/colleges">
                        <img
                          alt="slide1"
                          loading="lazy"
                          width={986}
                          height={763}
                          decoding="async"
                          data-nimg={1}
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs1.6193f08f.webp&w=1080&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs1.6193f08f.webp&w=2048&q=75 2x"
                          src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs1.6193f08f.webp&w=2048&q=75"
                        />
                      </a>
                    </div>
                  </div>
                </div>
                <div className="swiper-slide Hero_card__ynLLv">
                  <div className="Hero_heroMain__dP9sl">
                    <div>
                      <div className="Hero_searchContent__wSOwy">
                        <div className="Hero_searchContentBox__mTqoR">
                          <div>
                            <div className="counter_countercontainer__CiYqi">
                              <div className="counter_counter__W_2Wz">
                                <div className="counter_digit__L6cff">1</div>
                                <div className="counter_digit__L6cff">6</div>
                                <div className="counter_digit__L6cff">5</div>
                                <div className="counter_digit__L6cff">8</div>
                                <div className="counter_digit__L6cff">5</div>
                                <div className="counter_digit__L6cff">7</div>
                                <div className="counter_digit__L6cff">4</div>
                                <div className="counter_livetextbanner__Vykhp">
                                  · LIVE
                                </div>
                              </div>
                              <div className="counter_counterlabel__r7_fx">
                                STUDENTS COUNSELLED
                              </div>
                            </div>
                          </div>
                          <div>
                            <div className="SearchBar_searchWrapper__zmgjs">
                              <div className="SearchBar_searchBar__lYNNN">
                                <div
                                  className="SearchBar_Hero_input__VYpfM"
                                  variant="unstyled"
                                >
                                  Search for colleges and courses
                                </div>
                                <div className="SearchBar_Hero_iconContainer__psyvf">
                                  <svg
                                    stroke="currentColor"
                                    fill="none"
                                    strokeWidth={2}
                                    viewBox="0 0 24 24"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    className="SearchBar_Hero_icon__Wmwe2"
                                    height="1em"
                                    width="1em"
                                    xmlns="http://www.w3.org/2000/svg"
                                  >
                                    <circle cx={11} cy={11} r={8} />
                                    <line
                                      x1={21}
                                      y1={21}
                                      x2="16.65"
                                      y2="16.65"
                                    />
                                  </svg>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="Hero_textContainer__zTKJs">
                          <div className="Hero_Hero_text_1__tiOzY">
                            Konse College Me Admission Lena Sahi?
                          </div>
                          <div className="Hero_Hero_text_2__pG74x">
                            Ab Karo Har College Compare
                          </div>
                          <div className="Hero_hashtag__ky4R9">
                            <img
                              alt="hashtag"
                              loading="lazy"
                              width={559}
                              height={215}
                              decoding="async"
                              data-nimg={1}
                              style={{ color: "transparent" }}
                              srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fposter.ea45f88b.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fposter.ea45f88b.webp&w=1200&q=75 2x"
                              src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fposter.ea45f88b.webp&w=1200&q=75"
                            />
                          </div>
                          <div className="Hero_made__edb3d">
                            <div>MADE IN INDIA WITH</div>
                            <div>
                              <svg
                                stroke="currentColor"
                                fill="currentColor"
                                strokeWidth={0}
                                viewBox="0 0 512 512"
                                color="red"
                                style={{ color: "red" }}
                                height="1em"
                                width="1em"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M462.3 62.6C407.5 15.9 326 24.3 275.7 76.2L256 96.5l-19.7-20.3C186.1 24.3 104.5 15.9 49.7 62.6c-62.8 53.6-66.1 149.8-9.9 207.9l193.5 199.8c12.5 12.9 32.8 12.9 45.3 0l193.5-199.8c56.3-58.1 53-154.3-9.8-207.9z" />
                              </svg>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="Hero_imgCont__lhqFP">
                      <img
                        alt="slide1"
                        loading="lazy"
                        width={687}
                        height={696}
                        decoding="async"
                        data-nimg={1}
                        style={{ color: "transparent" }}
                        srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs2.35df2925.webp&w=750&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs2.35df2925.webp&w=1920&q=75 2x"
                        src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fs2.35df2925.webp&w=1920&q=75"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="swiper-pagination" />
            </div>
          </div>
        </div>
        <div className="AllCollegeCards_college_card_container__Ts4Zo">
          <div className="AllCollegeCards_college_grid_container__PA9Ro">
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/amity-university-online"
            >
              <img
                alt="AMITY UNIVERSITY"
                fetchpriority="high"
                loading="eager"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Famity.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Famity.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Famity.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                AMITY UNIVERSITY
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/chandigarh-university-online"
            >
              <img
                alt="CHANDIGARH UNI."
                fetchpriority="high"
                loading="eager"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fcu.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fcu.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fcu.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                CHANDIGARH UNI.
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/dy-patil-university-pune-online"
            >
              <img
                alt="DY PATIL UNIVERSITY"
                fetchpriority="high"
                loading="eager"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fdypatil.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fdypatil.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fdypatil.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                DY PATIL UNIVERSITY
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/iim-ahmedabad"
            >
              <img
                alt="IIM AHMEDABAD"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fiima.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fiima.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fiima.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                IIM AHMEDABAD
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/icfai-cdoe"
            >
              <img
                alt="ICFAI CDOE"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2FICFAi.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2FICFAi.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2FICFAi.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                ICFAI CDOE
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/jain-university-online"
            >
              <img
                alt="JAIN UNIVERSITY"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fjain.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fjain.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fjain.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                JAIN UNIVERSITY
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/online-manipal-jaipur"
            >
              <img
                alt="ONLINE MANIPAL"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fmanipal.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fmanipal.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fmanipal.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                ONLINE MANIPAL
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/upes-university-online"
            >
              <img
                alt="UPES UNIVERSITY"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fupes.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fupes.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fupes.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                UPES UNIVERSITY
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/op-jindal-global-university"
            >
              <img
                alt="O.P JINDAL UNI."
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fop-jindal.webp&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fop-jindal.webp&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fop-jindal.webp&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                O.P JINDAL UNI.
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/chitkara-university-online"
            >
              <img
                alt="CHITKARA UNI."
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fchitkara-logo.webp&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fchitkara-logo.webp&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fchitkara-logo.webp&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                CHITKARA UNI.
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/lpu-distance-education"
            >
              <img
                alt="LPU ONLINE"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Flpu.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Flpu.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Flpu.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                LPU ONLINE
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/imt-cdl-ghaziabad"
            >
              <img
                alt="IMT CDL"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fimt.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fimt.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fimt.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                IMT CDL
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/iim-raipur"
            >
              <img
                alt="IIM RAIPUR"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2Fiim-raipur.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2Fiim-raipur.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2Fiim-raipur.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                IIM RAIPUR
              </p>
            </a>
            <a
              className="AllCollegeCards_college_indi_card__d9m_r"
              href="/colleges/online-uttaranchal-university"
            >
              <img
                alt="ONLINE UU"
                loading="lazy"
                width={145}
                height={45}
                decoding="async"
                data-nimg={1}
                className="AllCollegeCards_college_indi_card_img__8_YF2"
                style={{ color: "transparent" }}
                srcSet="/_next/image?url=%2FcollegeIconImg%2FOnlineUU.png&w=256&q=75 1x, /_next/image?url=%2FcollegeIconImg%2FOnlineUU.png&w=384&q=75 2x"
                src="/_next/image?url=%2FcollegeIconImg%2FOnlineUU.png&w=384&q=75"
              />
              <p className="AllCollegeCards_college_indi_card_name__25zbO">
                ONLINE UU
              </p>
            </a>
          </div>
          <div className="AllCollegeCards_viewallcontainer__yIVAn">
            <a className="AllCollegeCards_viewAll__AEokp" href="/colleges">
              View All Colleges
            </a>
          </div>
        </div>
        <div
          className="BrowseCourse_home_browse_container__tQp9L"
          id="brCourses"
        >
          <div className="BrowseCourse_home_browse_heading_container__EJAFM">
            <div className="BrowseCourse_home_browse_heading_img_container__gqEkj">
              <div>
                <div>
                  <h2 className="BrowseCourse_home_browse_heading__10RSs">
                    EXPLORE OUR COURSES &amp; BE AWESOME
                  </h2>
                </div>
                <div>
                  <p className="BrowseCourse_home_browse_sub_heading__IbpwW">
                    Select the category and compare the university
                  </p>
                </div>
              </div>
            </div>
            <div />
          </div>
          <div>
            <div className="BrowseCourse_home_browse_course_container__PlZ71">
              <div className="BrowseCourse_home_browse_course_list_main_box__2D0BM">
                <div
                  className="BrowseCourse_home_course_list_single_box__d92j6 BrowseCourse_bgColor__zDC4o"
                  style={{ color: "white" }}
                >
                  <div className="BrowseCourse_browse_slide_individual_box__PeTNq">
                    <span>Online PG Programmes</span>
                  </div>
                </div>
                <div className="BrowseCourse_home_course_list_single_box__d92j6 ">
                  <div className="BrowseCourse_browse_slide_individual_box__PeTNq">
                    <span>Online UG Programmes</span>
                  </div>
                </div>
                <div className="BrowseCourse_home_course_list_single_box__d92j6 ">
                  <div className="BrowseCourse_browse_slide_individual_box__PeTNq">
                    <span>Diploma Courses</span>
                  </div>
                </div>
                <div className="BrowseCourse_home_course_list_single_box__d92j6 ">
                  <div className="BrowseCourse_browse_slide_individual_box__PeTNq">
                    <span>Executive Programmes</span>
                  </div>
                </div>
                <div className="BrowseCourse_home_course_list_single_box__d92j6 ">
                  <div className="BrowseCourse_browse_slide_individual_box__PeTNq">
                    <span>International Programmes</span>
                  </div>
                </div>
                <div className="BrowseCourse_home_course_list_single_box__d92j6 ">
                  <div className="BrowseCourse_browse_slide_individual_box__PeTNq">
                    <span>Free Courses</span>
                  </div>
                </div>
              </div>
              <div className="BrowseCourse_course_grid_box__h2DdU">
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/online-mba"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FOnlineMBA.png&w=256&q=75 1x, /_next/image?url=%2Fimages%2FOnlineMBA.png&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FOnlineMBA.png&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Online MBA
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/online-mca"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FOnlineMCA.png&w=256&q=75 1x, /_next/image?url=%2Fimages%2FOnlineMCA.png&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FOnlineMCA.png&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Online MCA
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/online-mcom"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FOnlineMCom.webp&w=256&q=75 1x, /_next/image?url=%2Fimages%2FOnlineMCom.webp&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FOnlineMCom.webp&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Online MCom
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/online-msc"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FOnlineMSc.webp&w=256&q=75 1x, /_next/image?url=%2Fimages%2FOnlineMSc.webp&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FOnlineMSc.webp&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Online MSc
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/online-ma"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FOnlineMA.webp&w=256&q=75 1x, /_next/image?url=%2Fimages%2FOnlineMA.webp&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FOnlineMA.webp&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Online MA
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/online-acca-mcom"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FOnlineMSc.webp&w=256&q=75 1x, /_next/image?url=%2Fimages%2FOnlineMSc.webp&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FOnlineMSc.webp&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          M.Com with ACCA
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/distance-mba"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FDistanceMBA.webp&w=256&q=75 1x, /_next/image?url=%2Fimages%2FDistanceMBA.webp&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FDistanceMBA.webp&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Distance MBA
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/distance-mca"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FDistanceMCA.webp&w=256&q=75 1x, /_next/image?url=%2Fimages%2FDistanceMCA.webp&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FDistanceMCA.webp&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Distance MCA
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/distance-mcom"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2Fimages%2FDistanceMCom.webp&w=256&q=75 1x, /_next/image?url=%2Fimages%2FDistanceMCom.webp&w=640&q=75 2x"
                          src="/_next/image?url=%2Fimages%2FDistanceMCom.webp&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Distance MCom
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}2 Years
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
                <div className="BrowseCourse_browse_course_card__6O_U1">
                  <a
                    style={{ textDecoration: "none" }}
                    href="/courses/master-of-library-and-information-science"
                  >
                    <div className="BrowseCourse_browse_course_card_top_container__4dzBP">
                      <div>
                        <img
                          alt="img"
                          loading="lazy"
                          width={200}
                          height={180}
                          decoding="async"
                          data-nimg={1}
                          className="BrowseCourse_browse_course_card_img__UCBw8"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2FbrowseCourse%2FMasterofLibraryandInformationScience.webp&w=256&q=75 1x, /_next/image?url=%2FbrowseCourse%2FMasterofLibraryandInformationScience.webp&w=640&q=75 2x"
                          src="/_next/image?url=%2FbrowseCourse%2FMasterofLibraryandInformationScience.webp&w=640&q=75"
                        />
                      </div>
                      <div>
                        <div className="BrowseCourse_browse_course_card_heading__7Cx3L">
                          Distance MLIS
                        </div>
                        <div className="BrowseCourse_browse_course_card_duration__mSOzf">
                          Duration: {/* */}1 Year
                        </div>
                      </div>
                    </div>
                  </a>
                  <div className="BrowseCourse_browse_course_card_view__wBC_X">
                    Read More
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="ImageSlider_sliderContainer__mlhoZ">
          <div className="ImageSlider_sliderWrapper__nAFir">
            <div className="ImageSlider_slideGrid__NwM1v">
              <a className="ImageSlider_slide__N_u17" href="/colleges">
                <div className="ImageSlider_imageWrapper__w1Nxx">
                  <img
                    alt="Slide 1"
                    fetchpriority="high"
                    width={640}
                    height={305}
                    decoding="async"
                    data-nimg={1}
                    className="ImageSlider_slideImage__V_iD1"
                    style={{ color: "transparent" }}
                    srcSet="/_next/image?url=%2Fimages%2Fslidercards%2F1.png&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fslidercards%2F1.png&w=1920&q=75 2x"
                    src="/_next/image?url=%2Fimages%2Fslidercards%2F1.png&w=1920&q=75"
                  />
                </div>
              </a>
              <a className="ImageSlider_slide__N_u17" href="/online-video-meet">
                <div className="ImageSlider_imageWrapper__w1Nxx">
                  <img
                    alt="Slide 2"
                    fetchpriority="high"
                    width={640}
                    height={305}
                    decoding="async"
                    data-nimg={1}
                    className="ImageSlider_slideImage__V_iD1"
                    style={{ color: "transparent" }}
                    srcSet="/_next/image?url=%2Fimages%2Fslidercards%2F2.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fslidercards%2F2.webp&w=1920&q=75 2x"
                    src="/_next/image?url=%2Fimages%2Fslidercards%2F2.webp&w=1920&q=75"
                  />
                </div>
              </a>
              <a className="ImageSlider_slide__N_u17" href="/ai-college-finder">
                <div className="ImageSlider_imageWrapper__w1Nxx">
                  <img
                    alt="Slide 3"
                    fetchpriority="high"
                    width={640}
                    height={305}
                    decoding="async"
                    data-nimg={1}
                    className="ImageSlider_slideImage__V_iD1"
                    style={{ color: "transparent" }}
                    srcSet="/_next/image?url=%2Fimages%2Fslidercards%2F3.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fslidercards%2F3.webp&w=1920&q=75 2x"
                    src="/_next/image?url=%2Fimages%2Fslidercards%2F3.webp&w=1920&q=75"
                  />
                </div>
              </a>
            </div>
          </div>
          <button
            className="ImageSlider_navButton__AOJvJ ImageSlider_prevButton__mDjpr"
            aria-label="Previous slide"
          >
            <svg
              stroke="currentColor"
              fill="currentColor"
              strokeWidth={0}
              viewBox="0 0 512 512"
              className="ImageSlider_navIcon__j99zf"
              height="1em"
              width="1em"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M256 504C119 504 8 393 8 256S119 8 256 8s248 111 248 248-111 248-248 248zM142.1 273l135.5 135.5c9.4 9.4 24.6 9.4 33.9 0l17-17c9.4-9.4 9.4-24.6 0-33.9L226.9 256l101.6-101.6c9.4-9.4 9.4-24.6 0-33.9l-17-17c-9.4-9.4-24.6-9.4-33.9 0L142.1 239c-9.4 9.4-9.4 24.6 0 34z" />
            </svg>
          </button>
          <button
            className="ImageSlider_navButton__AOJvJ ImageSlider_nextButton__qtGxb"
            aria-label="Next slide"
          >
            <svg
              stroke="currentColor"
              fill="currentColor"
              strokeWidth={0}
              viewBox="0 0 512 512"
              className="ImageSlider_navIcon__j99zf"
              height="1em"
              width="1em"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M256 8c137 0 248 111 248 248S393 504 256 504 8 393 8 256 119 8 256 8zm113.9 231L234.4 103.5c-9.4-9.4-24.6-9.4-33.9 0l-17 17c-9.4 9.4-9.4 24.6 0 33.9L285.1 256 183.5 357.6c-9.4 9.4-9.4 24.6 0 33.9l17 17c9.4 9.4 24.6 9.4 33.9 0L369.9 273c9.4-9.4 9.4-24.6 0-34z" />
            </svg>
          </button>
        </div>
        <div className="UsefulTools_usefultools_container__0usj7">
          <p className="UsefulTools_usefultools_subheading__P4IkQ">
            Making Your Life Easy
          </p>
          <h2 className="UsefulTools_usefultools_heading__T1fgC">
            USEFUL TOOLS
          </h2>
          <div className="UsefulTools_toolGrid__sVhb_">
            <a
              href="/sgpa-to-percentage-calculator"
              className="UsefulTools_toolLink__ztevN"
            >
              <div className="UsefulTools_toolCard__fe_ci">
                <div className="UsefulTools_iconWrapper__hAyFp">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 384 512"
                    className="UsefulTools_icon__wppRA"
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M109.25 173.25c24.99-24.99 24.99-65.52 0-90.51-24.99-24.99-65.52-24.99-90.51 0-24.99 24.99-24.99 65.52 0 90.51 25 25 65.52 25 90.51 0zm256 165.49c-24.99-24.99-65.52-24.99-90.51 0-24.99 24.99-24.99 65.52 0 90.51 24.99 24.99 65.52 24.99 90.51 0 25-24.99 25-65.51 0-90.51zm-1.94-231.43l-22.62-22.62c-12.5-12.5-32.76-12.5-45.25 0L20.69 359.44c-12.5 12.5-12.5 32.76 0 45.25l22.62 22.62c12.5 12.5 32.76 12.5 45.25 0l274.75-274.75c12.5-12.49 12.5-32.75 0-45.25z" />
                  </svg>
                </div>
                <h3 className="UsefulTools_toolName___I8lN">
                  SGPA to Percentage Calculator
                </h3>
                <div className="UsefulTools_buttonWrapper__hoyza">
                  <span className="UsefulTools_buttonText__LGVjG">
                    Use Calculator →
                  </span>
                </div>
              </div>
            </a>
            <a
              href="/how-to-convert-cgpa-to-percentage-cgpa-calculator"
              className="UsefulTools_toolLink__ztevN"
            >
              <div className="UsefulTools_toolCard__fe_ci">
                <div className="UsefulTools_iconWrapper__hAyFp">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 448 512"
                    className="UsefulTools_icon__wppRA"
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M400 0H48C22.4 0 0 22.4 0 48v416c0 25.6 22.4 48 48 48h352c25.6 0 48-22.4 48-48V48c0-25.6-22.4-48-48-48zM128 435.2c0 6.4-6.4 12.8-12.8 12.8H76.8c-6.4 0-12.8-6.4-12.8-12.8v-38.4c0-6.4 6.4-12.8 12.8-12.8h38.4c6.4 0 12.8 6.4 12.8 12.8v38.4zm0-128c0 6.4-6.4 12.8-12.8 12.8H76.8c-6.4 0-12.8-6.4-12.8-12.8v-38.4c0-6.4 6.4-12.8 12.8-12.8h38.4c6.4 0 12.8 6.4 12.8 12.8v38.4zm128 128c0 6.4-6.4 12.8-12.8 12.8h-38.4c-6.4 0-12.8-6.4-12.8-12.8v-38.4c0-6.4 6.4-12.8 12.8-12.8h38.4c6.4 0 12.8 6.4 12.8 12.8v38.4zm0-128c0 6.4-6.4 12.8-12.8 12.8h-38.4c-6.4 0-12.8-6.4-12.8-12.8v-38.4c0-6.4 6.4-12.8 12.8-12.8h38.4c6.4 0 12.8 6.4 12.8 12.8v38.4zm128 128c0 6.4-6.4 12.8-12.8 12.8h-38.4c-6.4 0-12.8-6.4-12.8-12.8V268.8c0-6.4 6.4-12.8 12.8-12.8h38.4c6.4 0 12.8 6.4 12.8 12.8v166.4zm0-256c0 6.4-6.4 12.8-12.8 12.8H76.8c-6.4 0-12.8-6.4-12.8-12.8V76.8C64 70.4 70.4 64 76.8 64h294.4c6.4 0 12.8 6.4 12.8 12.8v102.4z" />
                  </svg>
                </div>
                <h3 className="UsefulTools_toolName___I8lN">
                  CGPA to Percentage Calculator
                </h3>
                <div className="UsefulTools_buttonWrapper__hoyza">
                  <span className="UsefulTools_buttonText__LGVjG">
                    Use Calculator →
                  </span>
                </div>
              </div>
            </a>
            <a
              href="/tools/gmat-percentile-calculator"
              className="UsefulTools_toolLink__ztevN"
            >
              <div className="UsefulTools_toolCard__fe_ci">
                <div className="UsefulTools_iconWrapper__hAyFp">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 512 512"
                    className="UsefulTools_icon__wppRA"
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M496 384H64V80c0-8.84-7.16-16-16-16H16C7.16 64 0 71.16 0 80v336c0 17.67 14.33 32 32 32h464c8.84 0 16-7.16 16-16v-32c0-8.84-7.16-16-16-16zM464 96H345.94c-21.38 0-32.09 25.85-16.97 40.97l32.4 32.4L288 242.75l-73.37-73.37c-12.5-12.5-32.76-12.5-45.25 0l-68.69 68.69c-6.25 6.25-6.25 16.38 0 22.63l22.62 22.62c6.25 6.25 16.38 6.25 22.63 0L192 237.25l73.37 73.37c12.5 12.5 32.76 12.5 45.25 0l96-96 32.4 32.4c15.12 15.12 40.97 4.41 40.97-16.97V112c.01-8.84-7.15-16-15.99-16z" />
                  </svg>
                </div>
                <h3 className="UsefulTools_toolName___I8lN">
                  GMAT Percentile Calculator
                </h3>
                <div className="UsefulTools_buttonWrapper__hoyza">
                  <span className="UsefulTools_buttonText__LGVjG">
                    Use Calculator →
                  </span>
                </div>
              </div>
            </a>
            <a
              href="/tools/exams-eligibility-calculator"
              className="UsefulTools_toolLink__ztevN"
            >
              <div className="UsefulTools_toolCard__fe_ci">
                <div className="UsefulTools_iconWrapper__hAyFp">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 640 512"
                    className="UsefulTools_icon__wppRA"
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M622.34 153.2L343.4 67.5c-15.2-4.67-31.6-4.67-46.79 0L17.66 153.2c-23.54 7.23-23.54 38.36 0 45.59l48.63 14.94c-10.67 13.19-17.23 29.28-17.88 46.9C38.78 266.15 32 276.11 32 288c0 10.78 5.68 19.85 13.86 25.65L20.33 428.53C18.11 438.52 25.71 448 35.94 448h56.11c10.24 0 17.84-9.48 15.62-19.47L82.14 313.65C90.32 307.85 96 298.78 96 288c0-11.57-6.47-21.25-15.66-26.87.76-15.02 8.44-28.3 20.69-36.72L296.6 284.5c9.06 2.78 26.44 6.25 46.79 0l278.95-85.7c23.55-7.24 23.55-38.36 0-45.6zM352.79 315.09c-28.53 8.76-52.84 3.92-65.59 0l-145.02-44.55L128 384c0 35.35 85.96 64 192 64s192-28.65 192-64l-14.18-113.47-145.03 44.56z" />
                  </svg>
                </div>
                <h3 className="UsefulTools_toolName___I8lN">
                  Exam Eligibility Calculator
                </h3>
                <div className="UsefulTools_buttonWrapper__hoyza">
                  <span className="UsefulTools_buttonText__LGVjG">
                    Use Calculator →
                  </span>
                </div>
              </div>
            </a>
            <a
              href="/tools/neet-rank-predictor"
              className="UsefulTools_toolLink__ztevN"
            >
              <div className="UsefulTools_toolCard__fe_ci">
                <div className="UsefulTools_iconWrapper__hAyFp">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 512 512"
                    className="UsefulTools_icon__wppRA"
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M332.8 320h38.4c6.4 0 12.8-6.4 12.8-12.8V172.8c0-6.4-6.4-12.8-12.8-12.8h-38.4c-6.4 0-12.8 6.4-12.8 12.8v134.4c0 6.4 6.4 12.8 12.8 12.8zm96 0h38.4c6.4 0 12.8-6.4 12.8-12.8V76.8c0-6.4-6.4-12.8-12.8-12.8h-38.4c-6.4 0-12.8 6.4-12.8 12.8v230.4c0 6.4 6.4 12.8 12.8 12.8zm-288 0h38.4c6.4 0 12.8-6.4 12.8-12.8v-70.4c0-6.4-6.4-12.8-12.8-12.8h-38.4c-6.4 0-12.8 6.4-12.8 12.8v70.4c0 6.4 6.4 12.8 12.8 12.8zm96 0h38.4c6.4 0 12.8-6.4 12.8-12.8V108.8c0-6.4-6.4-12.8-12.8-12.8h-38.4c-6.4 0-12.8 6.4-12.8 12.8v198.4c0 6.4 6.4 12.8 12.8 12.8zM496 384H64V80c0-8.84-7.16-16-16-16H16C7.16 64 0 71.16 0 80v336c0 17.67 14.33 32 32 32h464c8.84 0 16-7.16 16-16v-32c0-8.84-7.16-16-16-16z" />
                  </svg>
                </div>
                <h3 className="UsefulTools_toolName___I8lN">
                  Neet Rank Predictor
                </h3>
                <div className="UsefulTools_buttonWrapper__hoyza">
                  <span className="UsefulTools_buttonText__LGVjG">
                    Use Calculator →
                  </span>
                </div>
              </div>
            </a>
            <a
              href="/tools/sgpa-to-cgpa-converter"
              className="UsefulTools_toolLink__ztevN"
            >
              <div className="UsefulTools_toolCard__fe_ci">
                <div className="UsefulTools_iconWrapper__hAyFp">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 448 512"
                    className="UsefulTools_icon__wppRA"
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M319.4 320.6L224 416l-95.4-95.4C57.1 323.7 0 382.2 0 454.4v9.6c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48v-9.6c0-72.2-57.1-130.7-128.6-133.8zM13.6 79.8l6.4 1.5v58.4c-7 4.2-12 11.5-12 20.3 0 8.4 4.6 15.4 11.1 19.7L3.5 242c-1.7 6.9 2.1 14 7.6 14h41.8c5.5 0 9.3-7.1 7.6-14l-15.6-62.3C51.4 175.4 56 168.4 56 160c0-8.8-5-16.1-12-20.3V87.1l66 15.9c-8.6 17.2-14 36.4-14 57 0 70.7 57.3 128 128 128s128-57.3 128-128c0-20.6-5.3-39.8-14-57l96.3-23.2c18.2-4.4 18.2-27.1 0-31.5l-190.4-46c-13-3.1-26.7-3.1-39.7 0L13.6 48.2c-18.1 4.4-18.1 27.2 0 31.6z" />
                  </svg>
                </div>
                <h3 className="UsefulTools_toolName___I8lN">
                  SGPA To CGPA Converter
                </h3>
                <div className="UsefulTools_buttonWrapper__hoyza">
                  <span className="UsefulTools_buttonText__LGVjG">
                    Use Calculator →
                  </span>
                </div>
              </div>
            </a>
            <a
              href="/tools/gpa-calculator-anna-university"
              className="UsefulTools_toolLink__ztevN"
            >
              <div className="UsefulTools_toolCard__fe_ci">
                <div className="UsefulTools_iconWrapper__hAyFp">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 512 512"
                    className="UsefulTools_icon__wppRA"
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M496 128v16a8 8 0 0 1-8 8h-24v12c0 6.627-5.373 12-12 12H60c-6.627 0-12-5.373-12-12v-12H24a8 8 0 0 1-8-8v-16a8 8 0 0 1 4.941-7.392l232-88a7.996 7.996 0 0 1 6.118 0l232 88A8 8 0 0 1 496 128zm-24 304H40c-13.255 0-24 10.745-24 24v16a8 8 0 0 0 8 8h464a8 8 0 0 0 8-8v-16c0-13.255-10.745-24-24-24zM96 192v192H60c-6.627 0-12 5.373-12 12v20h416v-20c0-6.627-5.373-12-12-12h-36V192h-64v192h-64V192h-64v192h-64V192H96z" />
                  </svg>
                </div>
                <h3 className="UsefulTools_toolName___I8lN">
                  GPA - Anna University
                </h3>
                <div className="UsefulTools_buttonWrapper__hoyza">
                  <span className="UsefulTools_buttonText__LGVjG">
                    Use Calculator →
                  </span>
                </div>
              </div>
            </a>
          </div>
        </div>
        <div className="Expert_Expert__Container__f2HSI">
          <div className="Expert_Expert__CardsContainer__DDdzT">
            <h2 className="Expert_Expert__text_1__YxPGj">
              CRAFT YOUR BETTER FUTURE WITH OUR EXPERTS
            </h2>
            <p className="Expert_Expert__text_2__6WHgU">
              LR has a team of 250+ experts who are there to assist you and give
              you the right guidance for your successful career ahead.
            </p>
            <div className="Expert_Expert__allCardsContainer__kKnmL">
              <div style={{ height: "100%" }}>
                <div className="Expert_Expert__cardContainer__2y7vz">
                  <div className="Expert_Expert__cardData__ocQ6N">
                    <div className="Expert_Expert__flexContainer__iCU0T">
                      <div className="Expert_Expert__imageContainer__zoZB6">
                        <img
                          alt="expertImage"
                          loading="lazy"
                          width={600}
                          height={373}
                          decoding="async"
                          data-nimg={1}
                          className="Expert_Expert__image__8wv_Z"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fkrishna.ea39407e.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fkrishna.ea39407e.webp&w=1200&q=75 2x"
                          src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fkrishna.ea39407e.webp&w=1200&q=75"
                        />
                        <div className="Expert_Expert__ratingContainer__UIlw9">
                          <div>
                            <div className="Expert_icon_container__lDnua">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="100%"
                                height="100%"
                                viewBox="0 0 24 24"
                                fill="url(#gradient)"
                              >
                                <linearGradient id="gradient">
                                  <stop offset="0%" stopColor="#FFC700" />
                                  <stop offset="100%" stopColor="#FF3D00" />
                                </linearGradient>
                                <path d="M12 2l2.4 7.2h7.6l-6 4.8 2.4 7.2-6-4.8-6 4.8 2.4-7.2-6-4.8h7.6z" />
                              </svg>
                            </div>
                          </div>
                          <p className="Expert_ExpertRatingValue__2LPF2">4.8</p>
                        </div>
                      </div>
                      <p className="Expert_Expert__card_text_1__NI_Zc">
                        Krishna
                      </p>
                      <p className="Expert_Expert__card_text_2__jOGNC">
                        Sr. Mentor for MBA
                      </p>
                      <p className="Expert_Expert__card_text_3__7CvA2">
                        3 Years Experience
                      </p>
                    </div>
                  </div>
                  <a href="/online-video-meet">
                    <button className="Expert_Expert__cardButton__cRBRJ">
                      Consult Now
                    </button>
                  </a>
                </div>
              </div>
              <div style={{ height: "100%" }}>
                <div className="Expert_Expert__cardContainer__2y7vz">
                  <div className="Expert_Expert__cardData__ocQ6N">
                    <div className="Expert_Expert__flexContainer__iCU0T">
                      <div className="Expert_Expert__imageContainer__zoZB6">
                        <img
                          alt="expertImage"
                          loading="lazy"
                          width={600}
                          height={373}
                          decoding="async"
                          data-nimg={1}
                          className="Expert_Expert__image__8wv_Z"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FMubashshir.759370d7.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2FMubashshir.759370d7.webp&w=1200&q=75 2x"
                          src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FMubashshir.759370d7.webp&w=1200&q=75"
                        />
                        <div className="Expert_Expert__ratingContainer__UIlw9">
                          <div>
                            <div className="Expert_icon_container__lDnua">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="100%"
                                height="100%"
                                viewBox="0 0 24 24"
                                fill="url(#gradient)"
                              >
                                <linearGradient id="gradient">
                                  <stop offset="0%" stopColor="#FFC700" />
                                  <stop offset="100%" stopColor="#FF3D00" />
                                </linearGradient>
                                <path d="M12 2l2.4 7.2h7.6l-6 4.8 2.4 7.2-6-4.8-6 4.8 2.4-7.2-6-4.8h7.6z" />
                              </svg>
                            </div>
                          </div>
                          <p className="Expert_ExpertRatingValue__2LPF2">4.5</p>
                        </div>
                      </div>
                      <p className="Expert_Expert__card_text_1__NI_Zc">
                        Mubashshir
                      </p>
                      <p className="Expert_Expert__card_text_2__jOGNC">
                        Sr. Mentor for MCA
                      </p>
                      <p className="Expert_Expert__card_text_3__7CvA2">
                        8 Years Experience
                      </p>
                    </div>
                  </div>
                  <a href="/online-video-meet">
                    <button className="Expert_Expert__cardButton__cRBRJ">
                      Consult Now
                    </button>
                  </a>
                </div>
              </div>
              <div style={{ height: "100%" }}>
                <div className="Expert_Expert__cardContainer__2y7vz">
                  <div className="Expert_Expert__cardData__ocQ6N">
                    <div className="Expert_Expert__flexContainer__iCU0T">
                      <div className="Expert_Expert__imageContainer__zoZB6">
                        <img
                          alt="expertImage"
                          loading="lazy"
                          width={600}
                          height={373}
                          decoding="async"
                          data-nimg={1}
                          className="Expert_Expert__image__8wv_Z"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FVaishaliThakur.1fd1e788.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2FVaishaliThakur.1fd1e788.webp&w=1200&q=75 2x"
                          src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FVaishaliThakur.1fd1e788.webp&w=1200&q=75"
                        />
                        <div className="Expert_Expert__ratingContainer__UIlw9">
                          <div>
                            <div className="Expert_icon_container__lDnua">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="100%"
                                height="100%"
                                viewBox="0 0 24 24"
                                fill="url(#gradient)"
                              >
                                <linearGradient id="gradient">
                                  <stop offset="0%" stopColor="#FFC700" />
                                  <stop offset="100%" stopColor="#FF3D00" />
                                </linearGradient>
                                <path d="M12 2l2.4 7.2h7.6l-6 4.8 2.4 7.2-6-4.8-6 4.8 2.4-7.2-6-4.8h7.6z" />
                              </svg>
                            </div>
                          </div>
                          <p className="Expert_ExpertRatingValue__2LPF2">4.4</p>
                        </div>
                      </div>
                      <p className="Expert_Expert__card_text_1__NI_Zc">
                        Vaishali
                      </p>
                      <p className="Expert_Expert__card_text_2__jOGNC">
                        Sr. Mentor for MBA
                      </p>
                      <p className="Expert_Expert__card_text_3__7CvA2">
                        6 Years Experience
                      </p>
                    </div>
                  </div>
                  <a href="/online-video-meet">
                    <button className="Expert_Expert__cardButton__cRBRJ">
                      Consult Now
                    </button>
                  </a>
                </div>
              </div>
              <div style={{ height: "100%" }}>
                <div className="Expert_Expert__cardContainer__2y7vz">
                  <div className="Expert_Expert__cardData__ocQ6N">
                    <div className="Expert_Expert__flexContainer__iCU0T">
                      <div className="Expert_Expert__imageContainer__zoZB6">
                        <img
                          alt="expertImage"
                          loading="lazy"
                          width={600}
                          height={373}
                          decoding="async"
                          data-nimg={1}
                          className="Expert_Expert__image__8wv_Z"
                          style={{ color: "transparent" }}
                          srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FDanishC.6a230675.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2FDanishC.6a230675.webp&w=1200&q=75 2x"
                          src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FDanishC.6a230675.webp&w=1200&q=75"
                        />
                        <div className="Expert_Expert__ratingContainer__UIlw9">
                          <div>
                            <div className="Expert_icon_container__lDnua">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="100%"
                                height="100%"
                                viewBox="0 0 24 24"
                                fill="url(#gradient)"
                              >
                                <linearGradient id="gradient">
                                  <stop offset="0%" stopColor="#FFC700" />
                                  <stop offset="100%" stopColor="#FF3D00" />
                                </linearGradient>
                                <path d="M12 2l2.4 7.2h7.6l-6 4.8 2.4 7.2-6-4.8-6 4.8 2.4-7.2-6-4.8h7.6z" />
                              </svg>
                            </div>
                          </div>
                          <p className="Expert_ExpertRatingValue__2LPF2">4.5</p>
                        </div>
                      </div>
                      <p className="Expert_Expert__card_text_1__NI_Zc">
                        Danish
                      </p>
                      <p className="Expert_Expert__card_text_2__jOGNC">
                        Sr. Mentor for Master of Arts
                      </p>
                      <p className="Expert_Expert__card_text_3__7CvA2">
                        4 Years Experience
                      </p>
                    </div>
                  </div>
                  <a href="/online-video-meet">
                    <button className="Expert_Expert__cardButton__cRBRJ">
                      Consult Now
                    </button>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="WhyUs_container__jG_LS">
          <div className="WhyUs_heroSection__fE4YP">
            <div className="WhyUs_leftSection__dvECi">
              <div className="WhyUs_experienceCard__mZ824">
                <div className="WhyUs_experienceContent__bU0un">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 256 256"
                    className="WhyUs_timeIcon__ie8GL"
                    height="1em"
                    width="1em"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M226.53,56.41l-96-32a8,8,0,0,0-5.06,0l-96,32A8,8,0,0,0,24,64v80a8,8,0,0,0,16,0V75.1L73.59,86.29a64,64,0,0,0,20.65,88.05c-18,7.06-33.56,19.83-44.94,37.29a8,8,0,1,0,13.4,8.74C77.77,197.25,101.57,184,128,184s50.23,13.25,65.3,36.37a8,8,0,0,0,13.4-8.74c-11.38-17.46-27-30.23-44.94-37.29a64,64,0,0,0,20.65-88l44.12-14.7a8,8,0,0,0,0-15.18ZM176,120A48,48,0,1,1,89.35,91.55l36.12,12a8,8,0,0,0,5.06,0l36.12-12A47.89,47.89,0,0,1,176,120ZM128,87.57,57.3,64,128,40.43,198.7,64Z" />
                  </svg>
                  <div>
                    <p>94,800+</p>
                    <span>Enrolled Students</span>
                  </div>
                </div>
              </div>
              <div className="WhyUs_imageWrapper__UfsSb">
                <img
                  alt="Student studying"
                  fetchpriority="high"
                  width={450}
                  height={320}
                  decoding="async"
                  data-nimg={1}
                  className="WhyUs_image__pqSOt"
                  style={{ color: "transparent" }}
                  srcSet="/_next/image?url=%2Fimages%2Fwhyus1.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fwhyus1.webp&w=1080&q=75 2x"
                  src="/_next/image?url=%2Fimages%2Fwhyus1.webp&w=1080&q=75"
                />
              </div>
            </div>
            <div className="WhyUs_rightSection__MwuY5">
              <div className="WhyUs_imageWrapper__UfsSb">
                <img
                  alt="Teacher helping student"
                  loading="lazy"
                  width={450}
                  height={320}
                  decoding="async"
                  data-nimg={1}
                  className="WhyUs_image__pqSOt"
                  style={{ color: "transparent" }}
                  srcSet="/_next/image?url=%2Fimages%2Fwhyus2.webp&w=640&q=75 1x, /_next/image?url=%2Fimages%2Fwhyus2.webp&w=1080&q=75 2x"
                  src="/_next/image?url=%2Fimages%2Fwhyus2.webp&w=1080&q=75"
                />
              </div>
              <div className="WhyUs_reviewCard__hFj11">
                <div className="WhyUs_avatarGroup__hCobu">
                  <div>
                    <svg
                      stroke="currentColor"
                      fill="currentColor"
                      strokeWidth={0}
                      viewBox="0 0 24 24"
                      className="WhyUs_timeIcon__ie8GL"
                      height="1em"
                      width="1em"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M12 2C6.486 2 2 6.486 2 12v4.143C2 17.167 2.897 18 4 18h1a1 1 0 0 0 1-1v-5.143a1 1 0 0 0-1-1h-.908C4.648 6.987 7.978 4 12 4s7.352 2.987 7.908 6.857H19a1 1 0 0 0-1 1V18c0 1.103-.897 2-2 2h-2v-1h-4v3h6c2.206 0 4-1.794 4-4 1.103 0 2-.833 2-1.857V12c0-5.514-4.486-10-10-10z" />
                    </svg>
                  </div>
                </div>
                <div>
                  <div className="WhyUs_reviewHeader__1G6H9">
                    <span>550+</span>
                  </div>
                  <p>Expert Counsellors</p>
                </div>
              </div>
            </div>
          </div>
          <div>
            <div className="WhyUs_aboutSection__oJ_R3">
              <p className="WhyUs_aboutHeader__kzyyu">
                Why We're The Front Runners
              </p>
              <h2 className="WhyUs_mainTitle__Y5jRZ">BEYOND COMPARISON</h2>
              <p className="WhyUs_description__y2f5_">
                Having over 8 years of experience in the EdTech industry, our
                seasoned professionals have helped over 94,800 individuals to
                realise their dreams. We help professionals find the right
                course based upon their interests and passions.
              </p>
            </div>
            <div>
              <div className="FeatureCards_grid__eohS1">
                <div className="FeatureCards_card__nomhR FeatureCards_blue__Zui4W">
                  <div className="FeatureCards_iconContainer__nJWtY">
                    <svg
                      stroke="currentColor"
                      fill="currentColor"
                      strokeWidth={0}
                      viewBox="0 0 512 512"
                      className="FeatureCards_icon__xRWQo"
                      height={24}
                      width={24}
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M496 384H64V80c0-8.84-7.16-16-16-16H16C7.16 64 0 71.16 0 80v336c0 17.67 14.33 32 32 32h464c8.84 0 16-7.16 16-16v-32c0-8.84-7.16-16-16-16zM464 96H345.94c-21.38 0-32.09 25.85-16.97 40.97l32.4 32.4L288 242.75l-73.37-73.37c-12.5-12.5-32.76-12.5-45.25 0l-68.69 68.69c-6.25 6.25-6.25 16.38 0 22.63l22.62 22.62c6.25 6.25 16.38 6.25 22.63 0L192 237.25l73.37 73.37c12.5 12.5 32.76 12.5 45.25 0l96-96 32.4 32.4c15.12 15.12 40.97 4.41 40.97-16.97V112c.01-8.84-7.15-16-15.99-16z" />
                    </svg>
                  </div>
                  <h3 className="FeatureCards_title__2rUbs">
                    Build Your Career
                  </h3>
                  <p className="FeatureCards_description__WKAA2">
                    Advance your career with flexible online courses.
                  </p>
                </div>
                <div className="FeatureCards_card__nomhR FeatureCards_yellow__UlRWd">
                  <div className="FeatureCards_iconContainer__nJWtY">
                    <svg
                      stroke="currentColor"
                      fill="currentColor"
                      strokeWidth={0}
                      viewBox="0 0 576 512"
                      className="FeatureCards_icon__xRWQo"
                      height={24}
                      width={24}
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M208 0c-29.9 0-54.7 20.5-61.8 48.2-.8 0-1.4-.2-2.2-.2-35.3 0-64 28.7-64 64 0 4.8.6 9.5 1.7 14C52.5 138 32 166.6 32 200c0 12.6 3.2 24.3 8.3 34.9C16.3 248.7 0 274.3 0 304c0 33.3 20.4 61.9 49.4 73.9-.9 4.6-1.4 9.3-1.4 14.1 0 39.8 32.2 72 72 72 4.1 0 8.1-.5 12-1.2 9.6 28.5 36.2 49.2 68 49.2 39.8 0 72-32.2 72-72V64c0-35.3-28.7-64-64-64zm368 304c0-29.7-16.3-55.3-40.3-69.1 5.2-10.6 8.3-22.3 8.3-34.9 0-33.4-20.5-62-49.7-74 1-4.5 1.7-9.2 1.7-14 0-35.3-28.7-64-64-64-.8 0-1.5.2-2.2.2C422.7 20.5 397.9 0 368 0c-35.3 0-64 28.6-64 64v376c0 39.8 32.2 72 72 72 31.8 0 58.4-20.7 68-49.2 3.9.7 7.9 1.2 12 1.2 39.8 0 72-32.2 72-72 0-4.8-.5-9.5-1.4-14.1 29-12 49.4-40.6 49.4-73.9z" />
                    </svg>
                  </div>
                  <h3 className="FeatureCards_title__2rUbs">
                    Grow Your Skills
                  </h3>
                  <p className="FeatureCards_description__WKAA2">
                    Develop expertise anytime, anywhere.
                  </p>
                </div>
                <div className="FeatureCards_card__nomhR FeatureCards_yellow__UlRWd">
                  <div className="FeatureCards_iconContainer__nJWtY">
                    <svg
                      stroke="currentColor"
                      fill="currentColor"
                      strokeWidth={0}
                      viewBox="0 0 640 512"
                      className="FeatureCards_icon__xRWQo"
                      height={24}
                      width={24}
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M622.34 153.2L343.4 67.5c-15.2-4.67-31.6-4.67-46.79 0L17.66 153.2c-23.54 7.23-23.54 38.36 0 45.59l48.63 14.94c-10.67 13.19-17.23 29.28-17.88 46.9C38.78 266.15 32 276.11 32 288c0 10.78 5.68 19.85 13.86 25.65L20.33 428.53C18.11 438.52 25.71 448 35.94 448h56.11c10.24 0 17.84-9.48 15.62-19.47L82.14 313.65C90.32 307.85 96 298.78 96 288c0-11.57-6.47-21.25-15.66-26.87.76-15.02 8.44-28.3 20.69-36.72L296.6 284.5c9.06 2.78 26.44 6.25 46.79 0l278.95-85.7c23.55-7.24 23.55-38.36 0-45.6zM352.79 315.09c-28.53 8.76-52.84 3.92-65.59 0l-145.02-44.55L128 384c0 35.35 85.96 64 192 64s192-28.65 192-64l-14.18-113.47-145.03 44.56z" />
                    </svg>
                  </div>
                  <h3 className="FeatureCards_title__2rUbs">
                    Personalized Guidance
                  </h3>
                  <p className="FeatureCards_description__WKAA2">
                    Get customised advice for your goals.
                  </p>
                </div>
                <div className="FeatureCards_card__nomhR FeatureCards_blue__Zui4W">
                  <div className="FeatureCards_iconContainer__nJWtY">
                    <svg
                      stroke="currentColor"
                      fill="currentColor"
                      strokeWidth={0}
                      viewBox="0 0 640 512"
                      className="FeatureCards_icon__xRWQo"
                      height={24}
                      width={24}
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M255.03 261.65c6.25 6.25 16.38 6.25 22.63 0l11.31-11.31c6.25-6.25 6.25-16.38 0-22.63L253.25 192l35.71-35.72c6.25-6.25 6.25-16.38 0-22.63l-11.31-11.31c-6.25-6.25-16.38-6.25-22.63 0l-58.34 58.34c-6.25 6.25-6.25 16.38 0 22.63l58.35 58.34zm96.01-11.3l11.31 11.31c6.25 6.25 16.38 6.25 22.63 0l58.34-58.34c6.25-6.25 6.25-16.38 0-22.63l-58.34-58.34c-6.25-6.25-16.38-6.25-22.63 0l-11.31 11.31c-6.25 6.25-6.25 16.38 0 22.63L386.75 192l-35.71 35.72c-6.25 6.25-6.25 16.38 0 22.63zM624 416H381.54c-.74 19.81-14.71 32-32.74 32H288c-18.69 0-33.02-17.47-32.77-32H16c-8.8 0-16 7.2-16 16v16c0 35.2 28.8 64 64 64h512c35.2 0 64-28.8 64-64v-16c0-8.8-7.2-16-16-16zM576 48c0-26.4-21.6-48-48-48H112C85.6 0 64 21.6 64 48v336h512V48zm-64 272H128V64h384v256z" />
                    </svg>
                  </div>
                  <h3 className="FeatureCards_title__2rUbs">Stay Updated</h3>
                  <p className="FeatureCards_description__WKAA2">
                    Keep up with the latest industry trends.
                  </p>
                </div>
              </div>
            </div>
            <a
              className="WhyUs_exploreButton__CFVYV"
              href="/online-video-meet"
              target="_blank"
              rel="noopener noreferrer"
            >
              <svg
                width={24}
                height={24}
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth={2}
              >
                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
              </svg>
              Connect with Experts
            </a>
          </div>
        </div>
        <div className="TestimonialSlider_container__Je36b">
          <div className="TestimonialSlider_wrapper__IvjzI">
            <div className="TestimonialSlider_header__znYD7">
              <h3 className="TestimonialSlider_label__z2WhH">
                Trusted Feedback
              </h3>
              <h2 className="TestimonialSlider_title__I_aMr">
                Check What Students Think Of Us
              </h2>
              <div className="TestimonialSlider_navigation__cQfS8">
                <button
                  className="TestimonialSlider_navButton__Cflns"
                  aria-label="Previous testimonial"
                >
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    strokeWidth={0}
                    viewBox="0 0 448 512"
                    height={24}
                    width={24}
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M257.5 445.1l-22.2 22.2c-9.4 9.4-24.6 9.4-33.9 0L7 273c-9.4-9.4-9.4-24.6 0-33.9L201.4 44.7c9.4-9.4 24.6-9.4 33.9 0l22.2 22.2c9.5 9.5 9.3 25-.4 34.3L136.6 216H424c13.3 0 24 10.7 24 24v32c0 13.3-10.7 24-24 24H136.6l120.5 114.8c9.8 9.3 10 24.8.4 34.3z" />
                  </svg>
                </button>
                <div className="TestimonialSlider_progressBar__pQdwb">
                  <div
                    className="TestimonialSlider_progressFill__TJBB8"
                    style={{ width: "40%" }}
                  />
                </div>
              </div>
            </div>
            <div className="TestimonialSlider_testimonialGrid__jr3sh">
              <div className="TestimonialSlider_testimonialCard__rAlFM TestimonialSlider_visible__zceg4">
                <div>
                  <div className="TestimonialSlider_imageContainer__qw6pl">
                    <img
                      src="/HomePage/Kartikeya.webp"
                      alt="Kartikeya Misra"
                      className="TestimonialSlider_avatar__tkuAI"
                    />
                  </div>
                  <div>
                    <h3 className="TestimonialSlider_name__gQxRY">
                      Kartikeya Misra
                    </h3>
                    <p className="TestimonialSlider_role__D4hYz">
                      (Altruist Technologies Pvt Ltd.)
                    </p>
                    <div className="TestimonialSlider_quote__D_DKH" />
                    <p className="TestimonialSlider_content__dEIEb">
                      Learning Routes Platform provided me with a seamless and
                      flexible learning experience. My coordinator, Mubbashir,
                      was extremely supportive and helped me in every possible
                      way throughout my entire course journey.
                    </p>
                  </div>
                </div>
              </div>
              <div className="TestimonialSlider_testimonialCard__rAlFM TestimonialSlider_visible__zceg4">
                <div>
                  <div className="TestimonialSlider_imageContainer__qw6pl">
                    <img
                      src="/HomePage/Manav-Dhodare.webp"
                      alt="Manav Dhodare"
                      className="TestimonialSlider_avatar__tkuAI"
                    />
                  </div>
                  <div>
                    <h3 className="TestimonialSlider_name__gQxRY">
                      Manav Dhodare
                    </h3>
                    <p className="TestimonialSlider_role__D4hYz">(HDFC Bank)</p>
                    <div className="TestimonialSlider_quote__D_DKH" />
                    <p className="TestimonialSlider_content__dEIEb">
                      Learning Routes provided me with a seamless learning
                      experience, it was like an excellent experience. Yash sir
                      helped me with each and every part of assignments and exam
                      schedules on time.
                    </p>
                  </div>
                </div>
              </div>
              <div className="TestimonialSlider_testimonialCard__rAlFM ">
                <div>
                  <div className="TestimonialSlider_imageContainer__qw6pl">
                    <img
                      src="/HomePage/Namita.webp"
                      alt="Namita Khandelwal"
                      className="TestimonialSlider_avatar__tkuAI"
                    />
                  </div>
                  <div>
                    <h3 className="TestimonialSlider_name__gQxRY">
                      Namita Khandelwal
                    </h3>
                    <p className="TestimonialSlider_role__D4hYz">
                      (Deloitte India)
                    </p>
                    <div className="TestimonialSlider_quote__D_DKH" />
                    <p className="TestimonialSlider_content__dEIEb">
                      It was a great learning experience for me, especially the
                      RM that had been assigned to me. My relationship manager
                      was always supportive, with a ready-to-respond attitude
                      towards every query.
                    </p>
                  </div>
                </div>
              </div>
              <div className="TestimonialSlider_testimonialCard__rAlFM ">
                <div>
                  <div className="TestimonialSlider_imageContainer__qw6pl">
                    <img
                      src="/HomePage/Piyush.webp"
                      alt="Piyush Sharma"
                      className="TestimonialSlider_avatar__tkuAI"
                    />
                  </div>
                  <div>
                    <h3 className="TestimonialSlider_name__gQxRY">
                      Piyush Sharma
                    </h3>
                    <p className="TestimonialSlider_role__D4hYz">
                      (Biocon Ltd.)
                    </p>
                    <div className="TestimonialSlider_quote__D_DKH" />
                    <p className="TestimonialSlider_content__dEIEb">
                      Thanks to Learning Routes Team for all the assistance and
                      support they provided in my 2-year PGDM programme. Overall
                      it was a good experience.
                    </p>
                  </div>
                </div>
              </div>
              <div className="TestimonialSlider_testimonialCard__rAlFM ">
                <div>
                  <div className="TestimonialSlider_imageContainer__qw6pl">
                    <img
                      src="/HomePage/bhavika.webp"
                      alt="Bhavika Shah"
                      className="TestimonialSlider_avatar__tkuAI"
                    />
                  </div>
                  <div>
                    <h3 className="TestimonialSlider_name__gQxRY">
                      Bhavika Shah
                    </h3>
                    <p className="TestimonialSlider_role__D4hYz">
                      (Dentsu Aegis Network)
                    </p>
                    <div className="TestimonialSlider_quote__D_DKH" />
                    <p className="TestimonialSlider_content__dEIEb">
                      My personal experience with Learning routes has been very
                      good. Very supportive and patient. Thank you for the all
                      the guidance.
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="News_News__Container__039ly">
          <p className="News_News__text_2__J_Ovz">
            What's{/* */} Trending in the Ed-Tech World?
          </p>
          <h2 className="News_News__text_1__53wqj">
            LATEST NEWS &amp; EVENTS{" "}
          </h2>
          <div className="News_News__allCardsContainer__n0_G_" />
        </div>
      </div>
    </div>
  </main>
  <div className="footer_footer__Container__brMaJ">
    <div className="footer_footer__row__kgs5H">
      <div className="footer_footer__col__4F7Mb">
        <a href="/">
          <img
            alt="logo"
            loading="lazy"
            width={600}
            height={207}
            decoding="async"
            data-nimg={1}
            className="footer_Footer__logo__xeANo"
            style={{ color: "transparent" }}
            srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FLR_logo.23fe17c1.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2FLR_logo.23fe17c1.webp&w=1200&q=75 2x"
            src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FLR_logo.23fe17c1.webp&w=1200&q=75"
          />
        </a>
        <div className="footer_Footer_LogoInfo_text_1__kVCZk">
          Embrace the convenience of learning and upskill with our extensive
          selection of online/distance degrees, diplomas, and certification
          courses, specially tailored for you. <br /> <br />
          Learning Routes tries to address the academic issues that students
          face. Learning Routes is India's first online platform that aims to
          bring together all of the online institutions on a single platform.
        </div>
        <div className="footer_Footer_LogoInfo_text_2__tZBCM">Follow us at</div>
        <div className="footer_Footer_LogoInfo_iconsContainer__lsNE7">
          <a href="https://www.instagram.com/learningroutes_official/">
            <div className="footer_Footer__icon_insta__dWcve">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 24 24"
                fontSize="25px"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M11.999 7.377a4.623 4.623 0 1 0 0 9.248 4.623 4.623 0 0 0 0-9.248zm0 7.627a3.004 3.004 0 1 1 0-6.008 3.004 3.004 0 0 1 0 6.008z" />
                <circle cx="16.806" cy="7.207" r="1.078" />
                <path d="M20.533 6.111A4.605 4.605 0 0 0 17.9 3.479a6.606 6.606 0 0 0-2.186-.42c-.963-.042-1.268-.054-3.71-.054s-2.755 0-3.71.054a6.554 6.554 0 0 0-2.184.42 4.6 4.6 0 0 0-2.633 2.632 6.585 6.585 0 0 0-.419 2.186c-.043.962-.056 1.267-.056 3.71 0 2.442 0 2.753.056 3.71.015.748.156 1.486.419 2.187a4.61 4.61 0 0 0 2.634 2.632 6.584 6.584 0 0 0 2.185.45c.963.042 1.268.055 3.71.055s2.755 0 3.71-.055a6.615 6.615 0 0 0 2.186-.419 4.613 4.613 0 0 0 2.633-2.633c.263-.7.404-1.438.419-2.186.043-.962.056-1.267.056-3.71s0-2.753-.056-3.71a6.581 6.581 0 0 0-.421-2.217zm-1.218 9.532a5.043 5.043 0 0 1-.311 1.688 2.987 2.987 0 0 1-1.712 1.711 4.985 4.985 0 0 1-1.67.311c-.95.044-1.218.055-3.654.055-2.438 0-2.687 0-3.655-.055a4.96 4.96 0 0 1-1.669-.311 2.985 2.985 0 0 1-1.719-1.711 5.08 5.08 0 0 1-.311-1.669c-.043-.95-.053-1.218-.053-3.654 0-2.437 0-2.686.053-3.655a5.038 5.038 0 0 1 .311-1.687c.305-.789.93-1.41 1.719-1.712a5.01 5.01 0 0 1 1.669-.311c.951-.043 1.218-.055 3.655-.055s2.687 0 3.654.055a4.96 4.96 0 0 1 1.67.311 2.991 2.991 0 0 1 1.712 1.712 5.08 5.08 0 0 1 .311 1.669c.043.951.054 1.218.054 3.655 0 2.436 0 2.698-.043 3.654h-.011z" />
              </svg>
            </div>
          </a>
          <a href="https://www.linkedin.com/company/kennis-learning-routes/">
            <div className="footer_Footer__icon_linkedin__n63hj">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 24 24"
                fontSize="20px"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <circle cx="4.983" cy="5.009" r="2.188" />
                <path d="M9.237 8.855v12.139h3.769v-6.003c0-1.584.298-3.118 2.262-3.118 1.937 0 1.961 1.811 1.961 3.218v5.904H21v-6.657c0-3.27-.704-5.783-4.526-5.783-1.835 0-3.065 1.007-3.568 1.96h-.051v-1.66H9.237zm-6.142 0H6.87v12.139H3.095z" />
              </svg>
            </div>
          </a>
          <a href="https://www.facebook.com/learningroutesLR">
            <div className="footer_Footer__icon_facebook__whpkx">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 24 24"
                fontSize="25px"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M13.397 20.997v-8.196h2.765l.411-3.209h-3.176V7.548c0-.926.258-1.56 1.587-1.56h1.684V3.127A22.336 22.336 0 0 0 14.201 3c-2.444 0-4.122 1.492-4.122 4.231v2.355H7.332v3.209h2.753v8.202h3.312z" />
              </svg>
            </div>
          </a>
          <a href="https://twitter.com/_learningroutes">
            <div className="footer_Footer__icon_twitter__W3K6m">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 24 24"
                fontSize="18px"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M8 2H1L9.26086 13.0145L1.44995 21.9999H4.09998L10.4883 14.651L16 22H23L14.3917 10.5223L21.8001 2H19.1501L13.1643 8.88578L8 2ZM17 20L5 4H7L19 20H17Z" />
              </svg>
            </div>
          </a>
          <a href="https://www.youtube.com/@LearningRoutesOfficial">
            <div className="footer_Footer__icon_utube__Xa6ET">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 576 512"
                fontSize="18px"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M549.655 124.083c-6.281-23.65-24.787-42.276-48.284-48.597C458.781 64 288 64 288 64S117.22 64 74.629 75.486c-23.497 6.322-42.003 24.947-48.284 48.597-11.412 42.867-11.412 132.305-11.412 132.305s0 89.438 11.412 132.305c6.281 23.65 24.787 41.5 48.284 47.821C117.22 448 288 448 288 448s170.78 0 213.371-11.486c23.497-6.321 42.003-24.171 48.284-47.821 11.412-42.867 11.412-132.305 11.412-132.305s0-89.438-11.412-132.305zm-317.51 213.508V175.185l142.739 81.205-142.739 81.201z" />
              </svg>
            </div>
          </a>
        </div>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_headerContainer__vQDCD">
          <text className="footer_Footer_section_heading__fsEMx">
            Top Universities
          </text>
        </div>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/amity-university-online"
        >
          Amity University
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/chandigarh-university-online"
        >
          Chandigarh University
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/dy-patil-university-pune-online"
        >
          DY Patil University
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/imt-cdl-ghaziabad"
        >
          IMT CDL
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/icfai-cdoe"
        >
          ICFAI CDOE
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/jain-university-online"
        >
          Jain University
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/online-manipal-jaipur"
        >
          Manipal Online
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/upes-university-online"
        >
          UPES University
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/op-jindal-global-university"
        >
          OP Jindal University
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/chitkara-university-online"
        >
          Chitkara University
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/lpu-distance-education"
        >
          LPU Online
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/iim-ahmedabad"
        >
          IIM Ahmedabad
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/iim-raipur"
        >
          IIM Raipur
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/colleges/online-uttaranchal-university"
        >
          Uttaranchal University
        </a>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_headerContainer__vQDCD">
          <text className="footer_Footer_section_heading__fsEMx">
            Online UG Programs
          </text>
        </div>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-bba"
        >
          Online BBA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-bca"
        >
          Online BCA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-bcom"
        >
          Online BCom
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-ba"
        >
          Online BA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-bba"
        >
          Distance BBA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-bca"
        >
          Distance BCA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-bcom"
        >
          Distance BCom
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-bsc"
        >
          Distance BSC
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-ba"
        >
          Distance BA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-bachelor-of-library-and-information-science"
        >
          Distance BLIS
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/online-distance-b-ed"
        >
          Distance B.Ed
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/b-tech-for-working-professionals"
        >
          B.Tech for Working Professionals
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/b-tech-lateral-entry"
        >
          B.Tech after Diploma
        </a>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_headerContainer__vQDCD">
          <text className="footer_Footer_section_heading__fsEMx">
            Top PG Courses
          </text>
        </div>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-mba"
        >
          Online MBA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-mca"
        >
          Online MCA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-mcom"
        >
          Online MCom
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-acca-mcom"
        >
          Online MCom with ACCA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-msc"
        >
          Online MSc
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/online-ma"
        >
          Online MA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-mba"
        >
          Distance MBA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-mca"
        >
          Distance MCA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/distance-mcom"
        >
          Distance MCom
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/master-of-library-and-information-science"
        >
          Distance MLIS
        </a>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_headerContainer__vQDCD">
          <text className="footer_Footer_section_heading__fsEMx">
            Online Executive Courses
          </text>
        </div>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-mba"
        >
          Executive MBA
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-pg-diploma-in-business-analytics"
        >
          EPGD in Business Analytics
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-programme-in-operations-and-supply-chain-management"
        >
          Operations &amp; Supply Chain
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-development-programme-in-strategic-management"
        >
          EDP Strategic Mgmt
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/chief-financial-officer-programme"
        >
          CFO Programme
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-programme-in-leadership-and-management"
        >
          Leadership &amp; Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-programme-in-general-management"
        >
          EP in General Mgmt
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-development-programme-in-strategic-human-resource-management"
        >
          EDP Strategic HR Mgmt
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-programme-in-project-management"
        >
          EP in Project Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/executive-programme-in-healthcare-service-management"
        >
          Healthcare Service Mgmt
        </a>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_heading__fsEMx">
          Get Right Career Guidance
        </div>
        <a
          className="footer_ctaboxlinks__5WueN"
          href="https://www.learningroutes.in/file/LR_Brochure.pdf"
        >
          <div className="footer_ctabox__61Q43">
            <div className="footer_leftsec__voM1P">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 384 512"
                className="footer_ctaicon__gkOGW"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M181.9 256.1c-5-16-4.9-46.9-2-46.9 8.4 0 7.6 36.9 2 46.9zm-1.7 47.2c-7.7 20.2-17.3 43.3-28.4 62.7 18.3-7 39-17.2 62.9-21.9-12.7-9.6-24.9-23.4-34.5-40.8zM86.1 428.1c0 .8 13.2-5.4 34.9-40.2-6.7 6.3-29.1 24.5-34.9 40.2zM248 160h136v328c0 13.3-10.7 24-24 24H24c-13.3 0-24-10.7-24-24V24C0 10.7 10.7 0 24 0h200v136c0 13.2 10.8 24 24 24zm-8 171.8c-20-12.2-33.3-29-42.7-53.8 4.5-18.5 11.6-46.6 6.2-64.2-4.7-29.4-42.4-26.5-47.8-6.8-5 18.3-.4 44.1 8.1 77-11.6 27.6-28.7 64.6-40.8 85.8-.1 0-.1.1-.2.1-27.1 13.9-73.6 44.5-54.5 68 5.6 6.9 16 10 21.5 10 17.9 0 35.7-18 61.1-61.8 25.8-8.5 54.1-19.1 79-23.2 21.7 11.8 47.1 19.5 64 19.5 29.2 0 31.2-32 19.7-43.4-13.9-13.6-54.3-9.7-73.6-7.2zM377 105L279 7c-4.5-4.5-10.6-7-17-7h-6v128h128v-6.1c0-6.3-2.5-12.4-7-16.9zm-74.1 255.3c4.1-2.7-2.5-11.9-42.8-9 37.1 15.8 42.8 9 42.8 9z" />
              </svg>{" "}
              Know More About Us
            </div>
            <div />
          </div>
        </a>
        <a
          className="footer_ctaboxlinks__5WueN"
          href="https://www.glassdoor.co.in/Overview/Working-at-Learning-Routes-EI_IE2599252.11,26.htm"
        >
          <div className="footer_ctabox__61Q43">
            <div className="footer_leftsec__voM1P">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 576 512"
                className="footer_ctaicon__gkOGW"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M320 32c0-9.9-4.5-19.2-12.3-25.2S289.8-1.4 280.2 1l-179.9 45C79 51.3 64 70.5 64 92.5V448H32c-17.7 0-32 14.3-32 32s14.3 32 32 32H96 288h32V480 32zM256 256c0 17.7-10.7 32-24 32s-24-14.3-24-32s10.7-32 24-32s24 14.3 24 32zm96-128h96V480c0 17.7 14.3 32 32 32h64c17.7 0 32-14.3 32-32s-14.3-32-32-32H512V128c0-35.3-28.7-64-64-64H352v64z" />
              </svg>{" "}
              Glassdoor
            </div>
            <div className="footer_rightsec2__viZrB">
              4.0{" "}
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 512 512"
                className="footer_staricon__shCsW"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M463 192H315.9L271.2 58.6C269 52.1 262.9 48 256 48s-13 4.1-15.2 10.6L196.1 192H48c-8.8 0-16 7.2-16 16 0 .9.1 1.9.3 2.7.2 3.5 1.8 7.4 6.7 11.3l120.9 85.2-46.4 134.9c-2.3 6.5 0 13.8 5.5 18 2.9 2.1 5.6 3.9 9 3.9 3.3 0 7.2-1.7 10-3.6l118-84.1 118 84.1c2.8 2 6.7 3.6 10 3.6 3.4 0 6.1-1.7 8.9-3.9 5.6-4.2 7.8-11.4 5.5-18L352 307.2l119.9-86 2.9-2.5c2.6-2.8 5.2-6.6 5.2-10.7 0-8.8-8.2-16-17-16z" />
              </svg>
            </div>
          </div>
        </a>
        <a
          className="footer_ctaboxlinks__5WueN"
          href="https://www.trustpilot.com/review/learningroutes.in"
        >
          <div className="footer_ctabox__61Q43">
            <div className="footer_leftsec__voM1P">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 576 512"
                className="footer_ctaicon__gkOGW"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M259.3 17.8L194 150.2 47.9 171.5c-26.2 3.8-36.7 36.1-17.7 54.6l105.7 103-25 145.5c-4.5 26.3 23.2 46 46.4 33.7L288 439.6l130.7 68.7c23.2 12.2 50.9-7.4 46.4-33.7l-25-145.5 105.7-103c19-18.5 8.5-50.8-17.7-54.6L382 150.2 316.7 17.8c-11.7-23.6-45.6-23.9-57.4 0z" />
              </svg>{" "}
              TrustPilot
            </div>
            <div className="footer_rightsec2__viZrB">
              4.5{" "}
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 512 512"
                className="footer_staricon__shCsW"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M463 192H315.9L271.2 58.6C269 52.1 262.9 48 256 48s-13 4.1-15.2 10.6L196.1 192H48c-8.8 0-16 7.2-16 16 0 .9.1 1.9.3 2.7.2 3.5 1.8 7.4 6.7 11.3l120.9 85.2-46.4 134.9c-2.3 6.5 0 13.8 5.5 18 2.9 2.1 5.6 3.9 9 3.9 3.3 0 7.2-1.7 10-3.6l118-84.1 118 84.1c2.8 2 6.7 3.6 10 3.6 3.4 0 6.1-1.7 8.9-3.9 5.6-4.2 7.8-11.4 5.5-18L352 307.2l119.9-86 2.9-2.5c2.6-2.8 5.2-6.6 5.2-10.7 0-8.8-8.2-16-17-16z" />
              </svg>
            </div>
          </div>
        </a>
        <a
          className="footer_ctaboxlinks__5WueN"
          href="https://www.ambitionbox.com/reviews/learning-routes-reviews"
        >
          <div className="footer_ctabox__61Q43">
            <div className="footer_leftsec__voM1P">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 512 512"
                className="footer_ctaicon__gkOGW"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M509.5 184.6L458.9 32.8C452.4 13.2 434.1 0 413.4 0H272v192h238.7c-.4-2.5-.4-5-1.2-7.4zM240 0H98.6c-20.7 0-39 13.2-45.5 32.8L2.5 184.6c-.8 2.4-.8 4.9-1.2 7.4H240V0zM0 224v240c0 26.5 21.5 48 48 48h416c26.5 0 48-21.5 48-48V224H0z" />
              </svg>{" "}
              AmbitionBox
            </div>
            <div className="footer_rightsec2__viZrB">
              4.3{" "}
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth={0}
                viewBox="0 0 512 512"
                className="footer_staricon__shCsW"
                height="1em"
                width="1em"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M463 192H315.9L271.2 58.6C269 52.1 262.9 48 256 48s-13 4.1-15.2 10.6L196.1 192H48c-8.8 0-16 7.2-16 16 0 .9.1 1.9.3 2.7.2 3.5 1.8 7.4 6.7 11.3l120.9 85.2-46.4 134.9c-2.3 6.5 0 13.8 5.5 18 2.9 2.1 5.6 3.9 9 3.9 3.3 0 7.2-1.7 10-3.6l118-84.1 118 84.1c2.8 2 6.7 3.6 10 3.6 3.4 0 6.1-1.7 8.9-3.9 5.6-4.2 7.8-11.4 5.5-18L352 307.2l119.9-86 2.9-2.5c2.6-2.8 5.2-6.6 5.2-10.7 0-8.8-8.2-16-17-16z" />
              </svg>
            </div>
          </div>
        </a>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_headerContainer__vQDCD">
          <text className="footer_Footer_section_heading__fsEMx">
            Diploma Courses
          </text>
        </div>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/pg-diploma-finance-accounting"
        >
          PGD Finance &amp; Acc
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/pg-diploma-data-science"
        >
          PGD Data Science
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/professional-diploma-digital-marketing"
        >
          Digital Marketing
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/pg-diploma-retail-management"
        >
          PGD Retail Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/pg-diploma-marketing-management"
        >
          PGD Marketing
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/diploma-hr-management"
        >
          PGD HR Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/banking-finance-management-diploma"
        >
          Banking &amp; Finance Mgmt
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/international-trade-management-diploma"
        >
          International Trade Mgmt
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/diploma-in-cyber-law"
        >
          Cyber Law
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/courses/pg-diploma-in-it"
        >
          PGD IT
        </a>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_headerContainer__vQDCD">
          <text className="footer_Footer_section_heading__fsEMx">
            Certification Programs
          </text>
        </div>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/data-analytics-certification"
        >
          Data Analytics
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/digital-marketing-certification"
        >
          Digital Marketing
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/cyber-security-certification"
        >
          Cyber Security
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/certification-in-ms-excel"
        >
          MS Excel
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/certification-in-ai-and-ml"
        >
          AL &amp; ML
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/certification-in-marketing"
        >
          Marketing
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/certification-in-leadership-skills"
        >
          Leadership Skills
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/project-management-certification"
        >
          Project Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/supply-chain-management-certification"
        >
          Supply Chain Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/freecourses/financial-accounting-certification"
        >
          Financial Accounting
        </a>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_headerContainer__vQDCD">
          <text className="footer_Footer_section_heading__fsEMx">
            Best Online Colleges
          </text>
        </div>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/top-10-online-mba-colleges-in-india"
        >
          Online MBA Colleges
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/top-distance-mba-colleges-in-india"
        >
          Distance MBA Colleges
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/best-executive-mba-colleges-india"
        >
          Executive MBA Colleges
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/top-online-bca-colleges-india"
        >
          Online BCA Colleges
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/online-ba-colleges-india"
        >
          Online BA Colleges
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/distance-online-bba-colleges"
        >
          Online BBA Colleges
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/top-online-bsc-colleges"
        >
          Online BSc Colleges
        </a>
      </div>
      <div className="footer_footer__col__4F7Mb">
        <div className="footer_Footer_section_headerContainer__vQDCD">
          <text className="footer_Footer_section_heading__fsEMx">
            Online MBA Specialisations
          </text>
        </div>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/distance-mba-in-business-management-2023-guide"
        >
          Business Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/online-mba-in-data-science"
        >
          Data Science
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/online-mba-entrepreneurship"
        >
          Entrepreneurship
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/mba-online-finance"
        >
          Financial Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/online-mba-in-healthcare"
        >
          Healthcare Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/online-mba-in-hr"
        >
          Human Resources Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/online-mba-international-business"
        >
          International Business
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/mba-marketing-online"
        >
          Marketing
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/distance-mba-in-operations-management-2023-guide"
        >
          Operations Management
        </a>
        <a
          className="footer_Footer_section_link__pMXOf"
          href="/blog/distance-mba-in-retail-management-the-complete-guide"
        >
          Retail Management
        </a>
      </div>
    </div>
  </div>
  <div className="footerbottom_footerbottom__Container__3yZAc">
    <div className="footerbottom_footerbottom__row__v6ZVx">
      <div className="footerbottom_footerbottom__col__4lnqQ">
        <img
          alt="logo"
          loading="lazy"
          width={400}
          height={400}
          decoding="async"
          data-nimg={1}
          className="footerbottom_qrcode__1H3_z"
          style={{ color: "transparent" }}
          srcSet="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fcareerqr.99c2a425.webp&w=640&q=75 1x, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fcareerqr.99c2a425.webp&w=828&q=75 2x"
          src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fcareerqr.99c2a425.webp&w=828&q=75"
        />
      </div>
      <div className="footerbottom_footerbottom__col__4lnqQ">
        <div className="footerbottom_Footerbottom_text_1__Ec9jQ" />
        <div className="footerbottom_Footerbottom_disclaimer__XNNH7">
          The intend of Learning Routes is to provide unbiased, precise
          information &amp; comparative guidance on Universities and its
          Programmes of Study to the Admission Aspirants. The contents of the
          Learning Routes Site, such as Texts, Graphics, Images, Blogs, Videos
          University Logos, and other materials (collectively, 'Content') are
          for information purpose only. The content is not intended to be a
          substitute for in any form on offerings of its academia partners.
          Infringing on intellectual property or associated rights is not
          intended or deliberately acted upon. The information provided by
          Learning Routes on www.learningroutes.in is for general information
          purposes only. All information on the site is provided in good faith
          with accuracy and to the best of our knowledge, however, we make no
          representation or warranty of any kind, express or implied, regarding
          the accuracy, adequacy, validity, reliability, or completeness of any
          information on the Site. Learning Routes &amp; its fraternity will not
          be liable for any errors, omissions, damages, or losses resultant, if
          any, from the usage of fit information.
        </div>
      </div>
      <div className="footerbottom_footerbottom__col__4lnqQ">
        <div className="footerbottom_Footerbottom_text_1__Ec9jQ">
          Contact Us:
        </div>
        <div className="footerbottom_Footerbottom_text_2__mwyDD">
          <svg
            stroke="currentColor"
            fill="currentColor"
            strokeWidth={0}
            viewBox="0 0 512 512"
            height="1em"
            width="1em"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fill="none"
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={32}
              d="M441.6 171.61L266.87 85.37a24.57 24.57 0 00-21.74 0L70.4 171.61A40 40 0 0048 207.39V392c0 22.09 18.14 40 40.52 40h335c22.38 0 40.52-17.91 40.52-40V207.39a40 40 0 00-22.44-35.78z"
            />
            <path
              fill="none"
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={32}
              d="M397.33 368L268.07 267.46a24 24 0 00-29.47 0L109.33 368m200-73l136-103m-384 0l139 105"
            />
          </svg>{" "}
          <a
            className="footerbottom_contactlinks__aVNGs"
            href="/cdn-cgi/l/email-protection#2d44434b426d41484c5f4344434a5f425859485e034443"
          >
            <span
              className="__cf_email__"
              data-cfemail="a0c9cec6cfe0ccc5c1d2cec9cec7d2cfd5d4c5d38ec9ce"
            >
              [email&nbsp;protected]
            </span>
          </a>
        </div>
        <div className="footerbottom_Footerbottom_text_3__huQ3M">
          <div className="footerbottom_Footerbottom_toptext_3__p0onr">
            Get Free Counselling
          </div>
          <a className="footerbottom_contactlinks__aVNGs" href="tel:9228165349">
            +91-9228165349
          </a>
        </div>
      </div>
    </div>
  </div>
  <div className="footer_Footer__bottomSectionContainer__L8VoB">
    <div className="footer_Footer__bottomSection_left__Q1LwX">
      <a className="footer_Footer__bottomSection_links__DUZG7" href="/">
        Home
      </a>
      <a className="footer_Footer__bottomSection_links__DUZG7" href="/aboutus">
        About us
      </a>
      <a
        className="footer_Footer__bottomSection_links__DUZG7"
        href="/life-at-learning-routes"
      >
        LR tag
      </a>
      <a className="footer_Footer__bottomSection_links__DUZG7" href="/career">
        Work with us
      </a>
      <a className="footer_Footer__bottomSection_links__DUZG7" href="/blog">
        Blogs
      </a>
      <a
        className="footer_Footer__bottomSection_links__DUZG7"
        href="/contact-us"
      >
        Contact Us
      </a>
    </div>
    <div className="footer_Footer__bottomSection_right__CSvmu">
      <text className="footer_Footer__bottomSection_links__DUZG7">
        Copyright 2024@LearningRoutes |
      </text>
      <a style={{ textDecoration: "none" }} href="/privacy-policy">
        <text className="footer_Footer__bottomSection_links__DUZG7">
          Privacy Policy
        </text>
      </a>
    </div>
  </div>
</div>;
